...
// A FormItem for entering a date relative to today or relative to some other date, or a 
// specific absolute date.  Typically used for filtering data by date.
// <P>
...
// +link{dataSourceField.type, data-type}.  For detailed information on working with dates, 
// times and datetimes, see the 
...
//    titleOrientation: "top",
    ...
    // Does this item's relative date value refer to the start or end of the chosen date? Useful
    ...
    //> @attr relativeDateItem.startDate (Date : null : IRW)
    // Limits the range of the popup +link{class:DateChooser}.  The default of null
    // causes the chooser's Year-picker to offer a range that starts 10 years before
    ...
    //> @attr relativeDateItem.endDate (Date : null : IRW)
    // Limits the range of the popup +link{class:DateChooser}.  The default of null
    // causes the chooser's Year-picker to offer a range that ends 10 years after the 
    // current selection.
    // @group appearance
    // @visibility external
    //<
...
isc.RelativeDateItem.addMethods({
    ...
    _createEditor : function(){
        ...
        // set up the default UI, according to the first of a specified this.value, a value 
        // specified in form.values, or finally the defaultValue (today)
        this.setValue(this.value || (this.form && this.form.getValues()[this.name]) || this.defaultValue);
    },
    ...
    setValue : function (value, allowNullValue) {
        ...
        if (value) {
            // if value isn't null, check if it's a shortcut (like $today) which isn't in the 
            // picker, oo a string (like +0d) that maps directly to shortcut that *is* in the
            // picker
            if (isc.DateUtil.isRelativeDateShortcut(value)) {
                if (!this.valueField.valueMap[value]) { 
                    // passed value is a valid relative-shortcut, but it isn't in the 
                    // presetOptions - update the value to the underlying value of the shortcut
                    value = isc.DateUtil.mapRelativeDateShortcut(value);
                }
            } else {
                var mappedShortcut = value && isc.DateUtil.mapToRelativeDateShortcut(value);
                if (mappedShortcut && mappedShortcut != value && this.valueField.valueMap[mappedShortcut]) {
                    // passed a relative-string that maps to a valid relative-shortcut, like 
                    // $tomorrow, which is present in the picker (presetOptions) - update the 
                    // value to the shortcut

                    
                    if (this.allowMapValueToShortcut) value = mappedShortcut;
                }
            }
        }
...
//                this.valueField.setValue("$today");
                ...
                key = period && parts.period == parts.period.toLowerCase() ? 
                        period.toLowerCase()+"_"+suffix : null;
    ...
    getRelativeDateObject : function (relativeDate) {
        return isc.DateUtil.getRelativeDateObject(relativeDate, this.rangePosition);
    },
    ...
    showPicker : function () {
        ...
        picker.startYear = this.getStartDate() ? this.getStartDate().getFullYear() : null;
        picker.endYear = this.getEndDate() ? this.getEndDate().getFullYear() : null;
    ...
    pickerDataChanged : function (picker) {
        ...
        var type = this.getType(),
            // this isDate includes dates and datetimes
            isDate = isc.SimpleType.inheritsFrom(type, "date") || isc.SimpleType.inheritsFrom(type, "datetime"),
    ...
    getStartDate : function () {
        // if null startDate, return null - see DateChooser.startYear 
    ...
    //<
    getEndDate : function () {
        // if null endDate, return null - see DateChooser.endYear
    ...
    parseDate : function (dateString, inputFormat) {
        ...
        var dataType = this.getType(),
            isDate = isc.SimpleType.inheritsFrom(dataType, "date") || isc.SimpleType.inheritsFrom(dataType, "datetime"),
