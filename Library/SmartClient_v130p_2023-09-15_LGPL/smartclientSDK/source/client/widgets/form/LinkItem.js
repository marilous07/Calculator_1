...
isc.LinkItem.addMethods({
    ...
    _getLinkElement : function () {
        if (!this.isReadOnly() || !this.isDrawn()) return null;
    ...
    _canFocus : function () {
        if (this.isReadOnly()) {
            // In readOnly mode if we have no value there's no
            // link element to focus on - otherwise we always return true
            return this.getValue() != null;
        } else {
            return this.Super("_canFocus", arguments);
        }
    },
    ...
    getTextBoxStyle : function () {
        ...
        var roTextBoxStyle = this.getReadOnlyTextBoxStyle();
        return (roTextBoxStyle ? this._getCellStyle(roTextBoxStyle) : null);
    },
    ...
    getLinkHTML : function (text) {
        ...
        if (title != "" || text != "") {
            ...
            if (this._needTextBoxTable()) {
                ...
                // We apply the padding to the text box table in FormItem.getTextBoxCSS() if
                // _needTextBoxTable() is true -- don't re-apply this padding if this is not the case        
    ...
    setElementValue : function (value, dataValue) {
