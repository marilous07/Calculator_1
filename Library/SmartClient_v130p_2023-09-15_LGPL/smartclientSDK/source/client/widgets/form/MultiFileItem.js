...
isc.MultiFileItem.addProperties({
    ...
    addFilesIcon: "[SKIN]MultiUploadItem/icon_add_files.png",
    ...
    removeFilesIcon: "[SKIN]MultiUploadItem/icon_remove_files.png",
...
formSaved : function (request, response, data) {
    ...
    if (this.picker) {
        ...
        var _this = this;
        this.picker.saveData(function () {_this.saveDataCallback();});
...
initWidget : function () {
    ...
    this.addAutoChild("cancelButton", {
        ...
        click : function () {
            this.picker.hide();
            // 'revertData' will clear and drop any newly added upload items
...
// Should we revert data [newly added upload items in the picker] when the user hits the cancel button?
// When enabled:
// - if editing an existing record the user has the option to save or cancel from the picker
//   save will upload the items immediately.
//   Cancel hides the editor and clears out any newly added / newly populated items so if you re-show the
//   picker they're not there
// - if editing a new record, the user has the option to "ok" or cancel the picker after adding/populating upload
//   forms
//   Hitting Okay shows the file names in the CanvasItem's listGrid but doesn't immediately upload
//   Hitting cancel cleas any newly added / newly populated items, but leaves those that were previously
//   chosen / okayed and are visible in the grid populated. If a user wants to clear those they have to
//   use the 'remove' icons
//   Saving the form in which the MFI is embedded will upload all the upload forms in the picker

revertDataOnHide:true,
...
addUploadField : function () {
    var form = this.creator ? this.creator.form : null;
    var binaryField = this.getBinaryFieldName();
        
        var form = this.createAutoChild("uploadForm", {
            checkFileAccessOnSubmit:form && form.checkFileAccessOnSubmit,
...
show : function () {
    var forms = this.getForms();
    if (forms) {
        // "newlyAdded" forms may be cleared if the user hits the cancel button
        
        var binaryField = this.getBinaryFieldName();
        for (var i = 0; i < forms.length; i++) {
            var undef;
            if (forms[i].getValue(binaryField) != undef) {
                forms[i].newlyAdded = false;
            }
        }
    }
    this.Super("show", arguments);
...
saveData : function (callback) {
    ...
    var _this = this,
        dsRequest = form.saveData(function (dsResponse, data, dsRequest) {
            _this.saveDataCallback(dsRequest, dsResponse, data);
        }, {
...
saveDataCallback : function (dsRequest, dsResponse, data) {
    ...
    if (this.callingFormItem && this.callingFormItem.fileUploaded) {
