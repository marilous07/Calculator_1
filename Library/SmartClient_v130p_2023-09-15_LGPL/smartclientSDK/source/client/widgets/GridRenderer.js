...
initWidget : function () {
    ...
    if (this.virtualScrolling && !this.showAllRows) {
        ...
        if (this.showCustomScrollbars == false) {
            ...
            // If we're in a UI where native scrollbars aren't visible by default but show up 
            // as you drag-scroll, we don't want to adjust the size of the body / content
            // to accommodate the scrollbar. Instead we'll allow the scrollbar to overlap the 
            // widget handle / content
            if (!isc.Element.visibleNativeScrollbars()) {
                this.floatingScrollbars = true;
            }
        }
    }
    if (this.showCustomScrollbars && this.floatingScrollbars) {
        this.suppressScrollbarGap = true;
    }
...
destroy : function () {
    ...
    // stop observing any embedded components
...
shouldShowAllCells : function (totalRows) {
    ...
    var totalCells = totalRows * Math.max(this.fields.length, 1);
...
//                        component + " went asynchronous", "printing");
	...
	if (isc.Browser.isDOM) {
        ...
        var cellHTML = [],
            ...
            cellIDSlot, divStartSlot = 21, cellValueSlot = 24;
        ...
        if (!fragment && !this.isPrinting && (isc.screenReader || this.writeOutCellElementId)) {
            cellHTML[19] = " id=";
            cellIDSlot = 20;
        }
		...
		for (var rowNum = startRow; rowNum < endRow; rowNum++) {
            ...
            // cellIDSlot is only set if screenreader mode is enabled or writeOutCellElementId is true
            if (cellIDSlot != null) {
            ...
            if (!fragment && !this.isPrinting  && (this.writeOutRowElementId || isc.screenReader)) {
            ...
            if (ariaEnabled) {
                ...
                if (this.getRowAriaState != null) {
                    rowState = this.getRowAriaState(rowNum, record, rowRole);
                ...
                if (rowState != null) {
                    if (cellIDSlot != null && isc.screenReader && rowState.labelledby == null) {
                        ...
                        if (this._getRowAriaLabelledBy) {
                            
                            var labelledby = this._getRowAriaLabelledBy(
                                                colNums, labelledByIDs, 
                                                this._getScreenReaderCellSeparatorID(), 
                                                this._getScreenReaderRowSeparatorID());           
                            if (labelledby) rowState.labelledby = labelledby;
                        }
                    }
			...
			for (var i = 0; i < colNums.length; i++) {
                ...
                if (singleCellSpan != null && (colNum == singleCellSpan[0])) {
                    ...
                    if (ariaEnabled) {
                        ...
                        var cellState = this.getCellAriaState && this.getCellAriaState(rowNum, colNum, record, cellRole);
                        if (cellState != null) {
			...
			if (this.isPrinting && record && this.grid) {
...
_putNativeFocusInRow : function (rowNum, colNum, suppressElementFocus) {
    // this.logWarn("Put native focus in row:" + [rowNum,colNum,suppressElementFocus]);
...
getFocusHandle : function () {
    if (isc.screenReader && !this._skipReturningNativeFocusRow) {
        var rowNum = this.getNativeFocusRow(), 
            colNum = this.canFocusInCells() ? this.getNativeFocusColumn() : null,
...
getMaxEmbeddedComponentHeight : function (record, rowNum) {
    var components = this.grid && this.grid._getEmbeddedComponents(record),
...
addEmbeddedComponent : function (component, record, rowNum, colNum, position, suppressRedraw) {
    ...
    if (this._embeddedComponents && this._embeddedComponents.contains(component)) {
        ...
        if (component.embeddedPosition == position && 
            component._currentRowNum == rowNum && component._currentColNum == colNum &&
            this.grid._hasEmbeddedComponents(record) &&
            this.grid._getEmbeddedComponents(record).contains(component)) 
        {
            return;
        }
    ...
    if (component.parentElement != this) {
        ...
        this._suppressAdjustOverflow = true;
        component.hide();
        // never autoDraw components when adding them - if the component may overflow, it will
        // be drawn later, after the sizing logic for how the cell might influence the widget 
        // size, but before the logic for how the component size might influence the row size
        this.addChild(component, null, false);
...
placeEmbeddedComponent : function (component, suppressRedraw) {
    ...
    if (position == this._$within) {
        ...
        if (!component.sizeWithExpansion && this.grid._hasEmbeddedComponents(record)) {
            ...
            // accommodate "expand" type components.
        ...
        if (component.overflow == isc.Canvas.VISIBLE && !component.isDrawn()) {
            // if the component may overflow and it isn't drawn, draw it now
            component.draw();
            // and update the height passed to snapToEdge(), so the comp centers in the height
...
removeEmbeddedComponent : function (record, component, suppressDeparent, suppressRedraw) {
    ...
    // don't clear the embeddedPosition when detaching components - there's no need
    // to do it, and there's no current way to restore it
    ...
    if (component.isExpansionComponent && this.grid.expansionComponentPoolingMode != "none") {
        ...
        if (this.grid.expansionComponentPoolingMode == "destroy") {
            ...
            if (exComp && !exComp.isStockComponent) {
...
getCellStyleIndex : function (record, rowNum, colNum) {
    ...
    // Not disabled - check for selected and/or over
        ...
        if (this.showSelectedStyle && this.selectionEnabled()) {
            var isSelected = this._showSelectedStyle(record, rowNum, colNum);
...
_showSelectedStyle : function (record, rowNum, colNum) {
    if (record && record._isGroup) this.logWarn("runing [gr version]");
...
//> @attr gridRenderer.writeOutRowElementId (Boolean : false : IRW)
// Should this Grid write an ID (obtained via +link{gridRenderer.getRowElementId()})
// into each row element?
// <P>
// Note that for integration with automated testing tools, we recommend using 
// +link{isc.AutoTest.getLocator()} and +link{isc.AutoTest.getElement()} to identify 
...
// <P>
// Note that if +link{isc.setScreenReaderMode(),screen reader mode} is enabled, row elements will
// be written out with a generated ID even if this property is false.<br>
// The default generated +link{listGrid.ariaRole,WAI-ARIA markup} for listGrids makes
// use of DOM IDs to identify rows and cells in various cases.
//
// @visibility external
//<
writeOutRowElementId:false,

//> @method gridRenderer.getRowElementId()  ([A])
// Get the DOM ID that should be used for a row element if 
// +link{isc.setScreenReaderMode(),screen reader mode} is enabled 
// or +link{gridRenderer.writeOutRowElementId} is true.
// <P>
// Note that for integration with automated testing tools, we recommend using 
// +link{isc.AutoTest.getLocator()} and +link{isc.AutoTest.getElement()} to identify 
// specific elements within the DOM wherever possible. These are more robust than simple
...
// @return (String) ID for the row element. This should be unique within the DOM.
// @visibility external
//<

getRowElementId : function (rowNum,physicalRowNum) {
    return this.ID + "_row" + rowNum;
},

//> @attr gridRenderer.writeOutCellElementId (Boolean : false : IRW)
// Should this Grid write an ID (obtained via +link{gridRenderer.getCellElementId()})
// into each cell element?
// <P>
// Note that for integration with automated testing tools, we recommend using 
// +link{isc.AutoTest.getLocator()} and +link{isc.AutoTest.getElement()} to identify 
// specific elements within the DOM wherever possible. These are more robust than simple
...
// using primary key and other approaches rather than relying on the generated DOM structure.
// <P>
// Note that if +link{isc.setScreenReaderMode(),screen reader mode} is enabled, cell elements will
// be written out with a generated ID even if this property is false.<br>
// The default generated +link{listGrid.ariaRole,WAI-ARIA markup} for listGrids makes
// use of DOM IDs to identify rows and cells in various cases.
//
// @visibility external
//<
writeOutCellElementId:false,

//> @method gridRenderer.getCellElementId()  ([A])
// Get the DOM ID that should be used for a cell element if 
// +link{isc.setScreenReaderMode(),screen reader mode} is enabled 
// or +link{gridRenderer.writeOutRowElementId} is true.
// <P>
// Note that for integration with automated testing tools, we recommend using 
...
// @return (String) ID for the cell element. This should be unique within the DOM.
// @visibility external
//<
getCellElementId : function (rowNum,physicalRowNum,colNum,physicalColNum) {
    return this.ID + "_cell" + rowNum + "_" + colNum;
},
...
// We need to separate the ability to natively focus in individual cells from 
// canSelectCells
// - In Screen Reader mode, a dev may want to configure a grid to allow a user to natively 
//   navigate through cells within the selected record [so they are read out cell by cell]
//   without forcing cell-selection model on
// - If the grid doesn't support selection at all we still want the ability to read its
//   content. An example of this is the Summary Row grid

screenReaderNavigateByCell:false, // picked up as a grid passthrough property, so typically overridden
canFocusInCells : function () {
    if (this.canSelectCells) return true;
    return isc.screenReader && this.screenReaderNavigateByCell;
},
...
_getViewportFillRows : function (scrollTop) {
    ...
    var divisor = this.grid && this.grid.recordComponentHeight || this.cellHeight;
    
    
    
    var firstVisible = Math.floor(scrollTop / divisor);
    if (firstVisible > this.getTotalRows()) {
        firstVisible = this.getTotalRows() - Math.ceil(viewportHeight / divisor);
    }
    ...
    if (this.autoFitData == "both" || this.autoFitData == "vertical") {
        ...
        if (totalHeight != null) {
            var maxRecordsByHeight = Math.ceil(totalHeight / divisor);
        ...
        var topOffset = scrollTop - (firstVisible * divisor);
        nRecords = Math.ceil((topOffset + viewportHeight) / divisor);        
    }
    ...
    if (this.startSpace) {
        var spaceRows = Math.floor(this.startSpace / divisor);
    ...
    if (this._rowOffset < 0) startCoord += Math.floor(this._rowOffset / divisor);
    if (startCoord < 0) startCoord = 0;
    var endCoord = startCoord + Math.ceil(viewportHeight / divisor);
...
// Documented at the ListGrid level
...
mouseOut : function () {
    ...
    if (this.showRollOver && this.preserveFocusStylingOnMouseOut && this._lastHiliteRow != null) {
        this.lastOverRow=this._lastHiliteRow;
        this.lastOverCol=this._lastHiliteCol || 0;
...
_cellHover : function (rowNum, colNum) {
	...
	if (this.showHover || (this.grid && this.grid.getCanHover())) {
        // _showHover now runs in more curcumstances, but has an extra restriction
        // to deal with grid.showHover:false with field.showHover:true, which allows
        // grid.showHover to work as a default, but always be overridable by fields
        this._showHover(record, rowNum, colNum, cellValueIsClipped);
    }
},
...
_showHover : function (record, rowNum, colNum, cellValueIsClipped) {
    ...
    if (!content) {
        ...
        if (this.cellHoverHTML._isPassthroughMethod && this.grid) {
            ...
            isCellHoverSuppressed = grid._isCellHoverSuppressed(rowNum, gridCol, field);
        }
...
handleSelectionChanged : function (record,state) {
    ...
    if (this.selectionChanged && this._isFrozenBody() == 0) return this.selectionChanged(record,state);
},
...
selectOnMouseDown : function (record, rowNum, colNum, keyboardGenerated) {
        ...
        !this._shouldSelectOnMouseUp(colNum))
    {
        ...
        } else if (this._deselectAllOnDisallowedSelectionEvent(record, rowNum, colNum, keyboardGenerated)) {
...
_shouldSelectOnMouseUp : function (colNum, isDragHandle) {
    var EH = this.ns.EH;
...
selectOnMouseUp : function (record, rowNum, colNum, keyboardGenerated) {
         ...
         (this._mouseDownRow == rowNum && this._mouseDownCol == colNum && !this._scrolledSinceMouseDown)))
    {
        if (this.canSelectRecord(record)) {
            ...
            if (this._shouldSelectOnMouseUp(colNum) && 
                this._deselectAllOnDisallowedSelectionEvent(record, rowNum, colNum, keyboardGenerated)) 
            {
                this.selectionManager.deselectAll();
            }
        }
...
// If a user mouseDowns, mouseUps or uses the keyboard to attempt to select a
// record where canSelectRecord() returns false, should we deselect all the other records?
...
//    this.logWarn("Logic to get col autoSize running:" + [columnNum, startRow, endRow]);
    ...
    if (isc.isA.Array(columnNum)) {
        ...
        // In some cases the size reported by getScrollWidth() can be too small
        // due to fractional reported coords
        // Use Math.ceil(getBoundingClientRect) on the  table instead so we round
        // fractional coords up rather than down.
        if (table != null && table.getBoundingClientRect) {
            returnVal = Math.ceil(table.getBoundingClientRect().width);
        } else {
...
setEndSpace : function (value) {
    ...
    this.endSpace = isNull ? null : value;
    if (!this.isDrawn()) return;
...
isc.GridRenderer._gridAPIs = {
    ...
    getRowAriaState : "rowNum,record,role",
    getCellRole : "rowNum,colNum,record",
    getCellAriaState : "rowNum,colNum,record,role",
    _getRowAriaLabelledBy : "colNums,labelledByIDs,cellSeparatorID,rowSeparatorID"
