...
//		@param	[secure]	(Boolean)	Secure setting for the cookie
//		@param	[samesite]	(String)	SameSite setting for the cookie. Defaults to "Lax" if not secure
//<
set : function (cName, value, path, domain, expiration, secure, samesite) {
	// call init again to refresh the list of cookies
	isc.Cookie.init();

    if (samesite == null && !secure) samesite = "Lax";
    var cookieString = cName + "=" + escape(value)
                        + (path ? ";path=" + path : "")
    ...
    //>DEBUG
    if (this.logIsDebugEnabled("Cookie")) {
        this.logDebug("Setting cookie:" + cookieString);
    }
    //<DEBUG
