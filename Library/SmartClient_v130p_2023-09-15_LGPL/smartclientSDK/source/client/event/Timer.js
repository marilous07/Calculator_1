...
// - repeating/looping timers that are always running [stored as a map of our action IDs to 'true' for quick lookup]
_backgroundRepeatTimers:{},

setTimeout : function (action, delay, units, frequentTimer, backgroundRepeatTimer) {
	...
	// If AEM is in 'suspended' state, ensure that when the timeout action fires we 
	// also suspend AEM if it's still active
	if (isc.Class._aemActive && isc.Class._aemSuspended) {
		if (this._aemExcludedActions == null) this._aemExcludedActions = {};
		this._aemExcludedActions[ID] = true;
	}
	...
	if (backgroundRepeatTimer) {
		this._backgroundRepeatTimers[ID] = true;
	}
    return tmrID;
},
...
_fireTimeout : function (ID, tmrID, delayedTmrID) {
	...
	if (this._backgroundRepeatTimers[ID]) delete this._backgroundRepeatTimers[ID];
	...
	if (isc.Class._aemActive && this._aemExcludedActions && this._aemExcludedActions[ID]) {
