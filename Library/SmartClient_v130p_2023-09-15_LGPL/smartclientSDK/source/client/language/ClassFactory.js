...
isc.addMethods(isc.ClassFactory, {
    ...
    _$iscPrefix: "isc.",
    _classTimes: {},
	_defineClass : function (className, superClass, interfaces, asInterface,
                             suppressSimpleNames, overwrite) 
    {
        ...
        if (!definingFramework && superClass && superClass._vbOnly && !isc.isReifySDK) {
        ...
        // bail out if we're configured to ignore duplicate class definitions
        if (existingObject != null && isc.ClassFactory.ignoreDuplicateClass) {
            if (window.isc.Log) {
                isc.Log.logInfo("Skipping creation of new class '" + className +
                                "' as it collides with an existing class or globally bound " +
                                "object (in Simple Names mode)");
            }
            return;
        }
        ...
        if (existingObject != null 
            ...
            // get warnings at VB startup when eg the Reify class clobbers the
            // Reify component schema.  Component Schema don't really need to be
        ...
        if (wd[object.ID] != object) {
            ...
            if (wd[object.ID] != null) {
                ...
                if (scClassInstance && wd[object.ID].destroy != null && 
                    ...
                    (!isc.FormItem || !isc.isA.FormItem(wd[object.ID]))) 
                {
    ...
    // Globally track whether Reify is loading a dataSource (and possibly its 
