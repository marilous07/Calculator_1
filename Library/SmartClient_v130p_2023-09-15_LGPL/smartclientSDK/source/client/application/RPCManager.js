...
//> @attr rpcRequest.reportDownloadErrorsAsDocuments (Boolean : null : IRW)
// Whether errors during download should be reported inside the document, rather than through
// the +link{rpcResponse.status,normal mechanism}.  If unset, this will be defaulted from
// +link{rpcManager.reportDownloadErrorsAsDocuments}.
// @visibility external
//<
...
// In browsers that support +externalLink{https://fetch.spec.whatwg.org/#http-cors-protocol,Cross-Origin Resource Sharing}
...
// (see <code>Access-Control-Allow-Credentials</code>),
...
// <P>
// Download requests do not fire a callback.<br>
// By default the request will use +link{rpcRequest.transport, transport}: "hiddenFrame".
// Developers may override this by explicitly setting <code>request.transport</code> to "xmlHttpRequest".
// This mode allows developers to send +link{rpcRequest.httpHeaders} to the server, but has some drawbacks:
// <ul><li>xmlHttpRequest download does not have a built-in progress bar to indicate download progress</li>
//     <li>The browser must hold the entire XHR response in memory, whereas with normal download
//         data is streamed to disk or to another application</li>
//     <li>This mode does not currently support +link{dsRequest.downloadToNewWindow}</li></ul>
//
// @visibility external
//<
...
// +link{RPCRequest.transport,transport:"xmlHttpRequest"}. See the 
// +link{rpcRequest.downloadResult,downloadResult} documentation for more details on 
...
// @see rpcRequest.reportDownloadErrorsAsDocuments
// @visibility external
//<
...
errorCodes : {
    ...
    //> @classAttr rpcResponse.STATUS_REQUIRED_CRITERIA_MISSING (int : -13 : R)
    // Indicates that an operation binding configured to require +link{OperationBinding.requiredCriterion} has received none.
    // @group statusCodes, constant
    // @visibility external
    //<
    STATUS_REQUIRED_CRITERIA_MISSING: -13,

    //> @classAttr rpcResponse.STATUS_CRITERIA_REQUIRED_ERROR (int : -13 : R)
    // @deprecated as of Smartclient 12.1, use +link{rpcResponse.STATUS_REQUIRED_CRITERIA_MISSING} instead
    // @group statusCodes, constant
    // @visibility external
    //<
    ...
    // return the English error property name if possible
    getErrorCodeKeyFromCode : function (errorCode) {
        var errorCodes = this.errorCodes;
        for (var key in errorCodes) {
            if (errorCodes[key] === errorCode) return key;
        }
        return errorCode;
    }
...
isc.RPCManager.addClassProperties({
    ...
    //> @classAttr rpcManager.reportDownloadErrorsAsDocuments (boolean : false : IRW)
    // Whether errors during download should be reported inside the document, rather than
    // through the +link{rpcResponse.status,normal mechanism}.  Establishes a default for
    // +link{rpcRequest.reportDownloadErrorsAsDocuments}.
    // @visibility external
    //<
...
// (See the +externalLink{https://fetch.spec.whatwg.org/#http-cors-protocol,W3C Cross-Origin Resource Sharing recommendation}).
...
// ---------------------------------------------------------------------------------------
    ...
    sendRequest : function (request) {
        ...
        // support backcompat mode for reporting request errors in download
        if (this.reportDownloadErrorsAsDocuments &&
            request.reportDownloadErrorsAsDocuments == null) 
        {
            request.reportDownloadErrorsAsDocuments = true;
        }
        if (request.reportDownloadErrorsAsDocuments) {
            var data = request.data;
            if (!data) request.data = data = {};
            data.reportDownloadErrorsAsDocuments = true;
        }
    ...
    canQueueRequest : function (request, transportIsExplicit) {
        ...
        // synchronousCallback requests cannot be sent as part of an async queue.  And there's no reason to handle
        // a queued synchronous block - as that's identical to sending "solo" synchronous requests
        if (request.synchronousCallback) return false;
    ...
    _addRequestToTransaction : function (request, transaction) {
        ...
        // propagate flag to transaction for later use
        if (request.synchronousCallback) {
            transaction.synchronousCallback = true;
        }
    ...
    doShowPrompt : function (transaction, prompt, promptDelay) {
        ...
        if (transaction.allClientOnly) return;
        ...
        if (promptDelay && !this._showedMask && !componentPromptOnly) {
            ...
            isc.EH.showClickMask(
                {target:this, methodName:"blockingRPCClick"}, 
                "hard", 
                unmaskedTargets ? unmaskedTargets.duplicate() : null, 
                "blockingRPC");
        ...
        var showPrompt = function () {
            if (transaction.promptStyle == "dialog" && prompt != null) {
                ...
                if (!_this._showedMask) {
                    ...
                    isc.EH.showClickMask(
                        {target:this, methodName:"blockingRPCClick"}, 
                        "hard", unmaskedTargets ? 
                        unmaskedTargets.duplicate() : null, 
                        "blockingRPC"
                    );
    ...
    // If the blockingRPC mask gets "stuck" up with no active transactions, dismiss it on click
    
    blockingRPCClick : function () {
        var activeTransactions = this.getActiveTransactions();
        if (activeTransactions == null || activeTransactions.length == 0) {
            
            isc.EH.delayCall("hideClickMask", ["blockingRPC"]);
        }
    },
    ...
    cancelQueue : function (transactionNum) {
        ...
        if (transaction.showPrompt && !transaction.allClientOnly) this.doClearPrompt(transaction);
    ...
    getTransaction : function (transaction, returnClearedTransactions) {
        ...
        try {
            ...
            // Unable to access properties of "transaction" - likely a window in another domain
            this.logDebug("getTransaction(): Unable to access properties of 'transaction' object passed in - error was:" + e);
            return null;
        }
        ...
        if (transaction && transaction.cleared && !returnClearedTransactions) return null;
    ...
    // @visibility external
    //<
    clearTransaction : function (transactionNum, skipPendingRpcs, maintainTracking) {
        var transaction = this.getTransaction(transactionNum);
        if (transaction == null) {
        ...
        transaction.maintainTracking = maintainTracking;
        if (!maintainTracking && !this._trackRPC && this._initializedTrackRPC) {
    ...
    pushRPCUpdate : function (transaction) {
        ...
        if (isc.debugTarget) {
            isc.debugTarget.pushRPCUpdate(transaction);
        }
        // once we push a cleared transaction to the debugMaster, remove it
        if (transaction.pushedToDebugMaster && transaction.cleared && !transaction.maintainTracking) {
            this._transactions.remove(transaction);
        }
    },
    ...
    safelyCombinePaths : function (a, b, c, d) {
        var combinedPath = a;
        for (var i = 1; i < arguments.length; i++) {
            if (!combinedPath.endsWith("/") && !arguments[i].startsWith("/")) {
                combinedPath += "/";
            }
    ...
    //     not intend.  <a href="https://en.wikipedia.org/wiki/Cross-site_request_forgery">Wikipedia</a></li>
    // </ul>
    //
    ...
    // code if placed into an HTML document.  An example of this in a SmartClient application
    ...
    // <p>
    // The way to prevent this kind of attack is +link{dataSourceField.escapeHTML}.  Setting
    ...
    // <p>
    // implementing a CSRF token strategy is really easy with SmartClient, especially if you 
    // are using the SmartClient server framework.  Since all data-related requests go to a 
    // single URL, specified with +link{RPCManager.actionURL}, you can just add your CSRF 
    // token to that URL in your bootstrap file.  Alternatively, you could use 
    // +link{RPCManager.transformRequest()} to add your token, if your authentication system 
    ...
    // SmartClient implements a technique called <i>domain synchronization</i> to ensure that 
    ...
    // Therefore, SmartClient passes the setting to the server with each "hiddenFrame"
    ...
    // a SmartClient application, where it shares a domain with another, non-SmartClient 
    // application that is vulnerable to XSS attacks - for example, a SmartClient app at 
    // <code>payments.example.com</code> and a non-SmartClient app at 
    // <code>enquiry.example.com</code>.  This attack would work by injecting an XSS payload 
    // into the vulnerable, non-SmartClient app at <code>enquiry.example.com</code>; this 
    // injected code would set the domain to "example.com", and then create an iframe that
    // targets the SmartClient app at "payments.example.com", passing <code>isc_dd</code> as
    ...
    // <ul>
    // <li>This exploit is only possible in these exact circumstances: ie, where a SmartClient 
    //     app is deployed to the same domain as a non-SmartClient app that is vulnerable to 
    //     XSS attacks</li>
    ...
    // <li>The attack is impossible if CSRF tokens are employed in the SmartClient app, since 
	...
	sendQueue : function (callback, prompt, URL, delay) {
		...
		if (delay && !transaction.synchronousCallback) {
            this.delayCall("_sendQueue", [callback,prompt,URL,transaction]);
        } else {
            return this._sendQueue(callback,prompt,URL,transaction);
        }
    },
    ...
    _sendQueue : function (callback,prompt,URL,transaction) {
		...
		if (allClientOnly) {
            ...
            if (transaction.synchronousCallback) {
                this._performTransactionReply(transaction, transaction.transactionNum);
            } else {
                this.delayCall("_performTransactionReply", [transaction.transactionNum], (this.clientOnlyResponseDelay || 0));
            }
			return request;
		}
	...
	performTransactionReply : function (transactionNum, results, wd) {
        ...
        if (transaction.transport == "xmlHttpRequest") {
            ...
            if (status > 299 || status < 200) { //error
                ...
                // if this was a relogin attempt, show the error inside the relogin form
                if (transaction.containsCredentials && !transaction.ignoreReloginMarkers &&
                    this._isHttpResponseCommFailure(status) && this.loginWindow)
                {
                    var form = this.loginWindow.loginForm;
                    form.setValue("loginFailure", this.reloginCommFailureMessage);
    ...
    //> @classAttr RPCManager.loginStatusCodeMarker (String : "" : IRWA)
    // String sequence which marks the response as a one which contains login status information.
    // <P>
    // The default loginStatusCodeMarker is the following string:
    // <code>"&lt;SCRIPT&gt;//'\"]]&gt;&gt;isc_"</code>
    //
    ...
    //> @classAttr RPCManager.loginRequiredMarker (String : "" : IRWA)
    // Marker the system will look for in XHTP responses in order to detect when login is required.
    // The default loginRequiredMarker is the following string:<br>
    // <code>"&lt;SCRIPT&gt;//'\"]]&gt;&gt;isc_loginRequired"</code>
    // <P>
    // As described in +link{group:relogin}, if this snippet is encountered in the response
    // to a SmartClient RPC request (with standard transport <code>"xmlHttpRequest"</code>), 
    // the RPCManager will suspend the current transaction and fire the +link{loginRequired()} 
    // notification.
    // <P>
    ...
    //> @classAttr RPCManager.loginSuccessMarker (String : "" : IRWA)
    //
    // Marker the system will look for in order to detect when login was successful.
    // <P>
    // The default loginSuccessMarker is the following string:<br>
    // <code>"&lt;SCRIPT&gt;//'\"]]&gt;&gt;isc_loginSuccess"</code>
    //
    // @see RPCManager.loginRequiredMarker
    //
    // @group relogin
    ...
    //> @classAttr RPCManager.maxLoginAttemptsExceededMarker (String : "" : IRWA)
    //
    // Marker the system will look for in order to detect when the number of maximum logins was exceeded.
    // <P>
    // The default maxLoginAttemptsExceededMarker is the following string:
    // <code>"&lt;SCRIPT&gt;//'\"]]&gt;&gt;isc_maxLoginAttemptsExceeded"</code>
    //
    // @see RPCManager.loginRequiredMarker
    //
    // @group relogin
	...
	_performTransactionReply : function (transactionNum) {
        ...
        if (this._shouldRecordTimingData()) {
        ...
        if (!transaction.suspended && !transaction.abortCallbacks) {
            ...
            if (this.transactionComplete != null) {
                ...
                this.clearTransaction(transactionNum, true, true);
            } else {
                // Third param maintains transaction in this._transactions
                // this allows transactionProcessingComplete() to do meaningful
                // things with the transactionNum.
                // We'll explicitly delete the transaction from our _transactions list when
                // transactionProcessingComplete is done
                this.clearTransaction(transactionNum, false, true);
            }
        }
        ...
        this._transactionProcessingComplete(transactionNum, transaction);
	},

    // Support adding 'processing complete callbacks' to fire when we fully complete
    // processing any transaction. This can be useful to extract timing data
    // during automated testing
    
    //> @classMethod RPCManager.addProcessingCompleteCallback()
    // This method will register a callback to fire every time the processing of an RPC 
    // transaction is fully complete, including any +link{dsRequest.callback,request-level} 
    // or +link{RPCManager.sendQueue(),queue-level} user callbacks.
    // @param callback (Callback) Callback to fire when processing is
    //  complete. This takes a single parameter "transactionNum"
    // @return (integer) Identifier for the registered callback. May
    //  be passed to +link{removeProcessingCompleteCallback()} to 
    //  unregister this callback.
    // @visibility internal in 13.0
    // @see RPCManager.removeProcessingCompleteCallback()
    // @see RPCManager.getTimingData()
    //<
    _processingCompleteCallbackIndex:0,
    _processingCompleteCallbacks:{},
    addProcessingCompleteCallback : function (callback) {
        var index = this._processingCompleteCallbackIndex++;
        this._processingCompleteCallbacks[index] = callback;
        return index;
    },

    //> @classMethod RPCManager.removeProcessingCompleteCallback()
    // Unregister the +link{addProcessingCompleteCallback(),processingComplete} callback
    // associated with the parameter index
    // @param integer (index) The identifying index returned from +link{addProcessingCompleteCallback()}
    // @visibility internal in 13.0
    //<
    removeProcessingCompleteCallback : function (index) {
        delete this._processingCompleteCallbacks[index];
    },
    

    // _transactionProcessingComplete() differs from transactionComplete() - 
    // this fires after all user callbacks have completed and includes timing data
    _transactionProcessingComplete : function (transactionNum, transaction) {

        var callbacks = this._processingCompleteCallbacks;
        for (var index in callbacks) {
            
            this.fireCallback(callbacks[index], "transactionNum", [transactionNum]);
        }

        // maintainTracking flag was explcitly set to ensure we don't
        // remove from _transactions until this method fires - clear that now.
        delete transaction.maintainTracking;

        // Remove from this._transactions if appropriate
        
        if (transaction.cleared) {
            if ((!this._trackRPC && this._initializedTrackRPC) || transaction.pushedToDebugMaster) {
                this._transactions.remove(transaction);
            }
        }
    },

    _shouldRecordTimingData : function () {
        return this._timingDataEnabled || isc.Log.logIsInfoEnabled("RpcTabTiming");
    },
    _timingDataEnabled:false,

    //> @classMethod RPCManager.setTimingDataEnabled()
    // Pass <code>true</code> or <code>false</code> to switch the gathering of timing metrics 
    // on and off programmatically.  The timing data thus gathered can be viewed in the "Timing"
    // section of the +link{group:devConsoleRPCTab,Developer Console RPC tab}, or retrieved by
    // your own code by use of the +link{RPCManager.getTimingData()} API.
    // @param enabled (boolean) Whether to switch timing data on or off
    // @visibility internal in 13.0
    //<
    setTimingDataEnabled : function (enabled) {
        enabled = !!enabled;
        if (this._timingDataEnabled != enabled) {
            this._timingDataEnabled = enabled;
            // Enable the log category or it will not gather client timings
            isc.Log.setPriority("RpcTabTiming", enabled ? isc.Log.INFO : isc.Log.defaultPriority);
        }
    },
    ...
    // <code>"Transaction with <i>n</i> operation(s). [dsName1.operationType; dsName2.operationType; ...]"</code>
    // 
    // @param transactionNum (integer) The number of the transaction to return timing data for
    // @return (String) transaction description
    //<
    getTransactionDescription : function (transactionNum) {

        var transaction = this.getTransaction(transactionNum, true);
        if (transaction == null) return "Unable to retrieve transaction";
        var tRecord = this.convertToTransactionRecord(transaction);
        if (tRecord.rpcs == null || tRecord.rpcs.length == 0) {
            return "Empty Transaction"
        }
        var rpcDescriptions = [];
        for (var i = 0; i < tRecord.rpcs.length; i++) {
            var rpc = tRecord.rpcs[i];
            if (rpc.ds != null) {
                rpcDescriptions.add(rpc.ds + "." + rpc.operationType);
            } else {
                rpcDescriptions.add("rpcRequest")
            }
        }
        return ("Transaction with " + tRecord.rpcs.length + " operation(s). [" + rpcDescriptions.join("; ") + "]");
    },

    //> @method Callbacks.FormattedTimingDataCallback
    // Callback fired after a call to +link{RPCManager.getFormattedTimingData(}} to report the
    // formatted timing data.
    //
    // @param data (String) The formatted timing data.
    // @visibility internal in 13.0
    //<

    //> @method Callbacks.TimingDataCallback
    // Callback fired after a call to +link{RPCManager.getTimingData(}} to report gathered
    // timing data.
    //
    // @param data (Object) The root object of the timing data tree (note, this is a plain JS
    //                      object, not an instance of the SmartClient Tree class)
    // @visibility internal in 13.0
    //<
    ...
    // +link{RPCManager.getTimingData()} API.  Intended for use in log messages and other 
    // non-interactive, information-only settings.  For more advanced usages, call 
    // <code>getTimingData()</code> and work directly with the tree structure that API returns.
    // <p>
    // Please read the <code>getTimingData()</code> documentation for some important remarks
    // about the availability of timing data, depending on where you are calling it from.
    // These remarks apply equally to <code>getFormattedTimingData()</code>
    // @return (String) Formatted text visualising the tree of timing data, if the timing data
    //                  can be retrieved synchronously
    // @param transactionNum (integer) The number of the transaction to return timing data for
    // @param [callback] (FormattedTimingDataCallback) A callback to invoke, passing the
    //                                                 formatted timing data.  This parameter
    //                                                 is optional, but it is necessary if
    //                                                 timing data cannot be obtained
    //                                                 synchronously, as described in the
    //                                           <code>getTimingData()</code> documentation
    // @param [operationNo] (integer) Optionally, the index of a specific operation in the 
    //                                overall transaction. Only pass this parameter if you want
    //                                to limit the returned data to a single request
    // @param [includeClient] (boolean) Whether to include client-side timing data.  Optional
    //                                  defaults to true
    // @param [includeServer] (boolean) Whether to include server-side timing data.  Optional, 
    //                                  defaults to true
    // @param [maxDepth] (integer) The maximum depth to descend into the tree.  The lowest 
    //                             meaningful value for this parameter is 1 - anything less 
    //                             will simply cause the API to return an empty string.  This 
    //                             parameter is optional; if it not passed, we default to 
    //                             "no limit"
    // @visibility internal in 13.0
    //<
    getFormattedTimingData : function(transactionNum, callback, operationNo, includeClient, includeServer, maxDepth) {
        var data = this.getTimingData(transactionNum, function(retval) {
            var fmt = isc.RPCManager._getFormatted(retval, 0, maxDepth);
            if (callback) {
                callback(fmt);
            }
        }, operationNo, includeClient, includeServer);
        if (isc.isA.String(data)) {
            return data;
        } else {
            return this._getFormatted(data.root, 0, maxDepth);
        }
    },

    _getFormatted : function(data, depth, maxDepth) {
        var undef;
        if (maxDepth != undef && depth >= maxDepth) return "";
        var str = "-------------------------------------------------------".substring(0, depth*2);
        str += " ";
        if (isc.isAn.Object(data)) {
            var timing = (data.start ? (data.end ? (data.elapsed + "ms") 
                                                 : "End time not recorded") 
                                     : "Start time not recorded");
            str += data.name + " (" + timing + ")\n";
            if (data.children) {
                for (var i = 0; i < data.children.length; i++) {
                    str += this._getFormatted(data.children[i], depth+1, maxDepth);
                }
            }
        } else {
            str += "Unexpected value: " + data;
        }
        return str;
    },

    //> @classMethod RPCManager.getTimingData()
    // Returns timing data gathered for the transaction number passed in.  The timing data is
    // an instance of +link{class:Tree,the SmartClient Tree class}, where parent nodes 
    // encapsulate the timings of their descendants.  This timing data is what drives the 
    // "Timing" view of the +link{group:devConsoleRPCTab,Developer Console RPC tab}.  Note, 
    // the returned <code>Tree</code> has +link{Tree.modelType,modelType} "children", so it 
    // can easily be traversed manually by simply following the nodes' "children" property 
    // recursively from the tree's "root" element.
    // <p>
    // Each node in the tree contains the following properties:<table width="700" style="border:1px solid black">
    // <tr><td width="100"><code>name</code></td><td>A descriptive name</td></tr>
    // <tr><td><code>start</code></td><td>The start time, in milliseconds since the epoch, recorded by
    // either the client or the server, depending on where this data was gathered.  See the
    // important notes at the bottom of the +link{group:devConsoleRPCTab,Developer Console RPC}
    // article regarding the relevance of the <code>start</code> and <code>end</code> time 
    // properties</td></tr>
    // <tr><td><code>end</code></td><td>The end time, in milliseconds since the epoch.  See 
    // <code>start</code>, above</td></tr>
    // <tr><td><code>elapsed</code></td><td>The elapsed time in milliseconds for this operation</td></tr>
    // <tr><td><code>children</code></td><td>The list of this node's direct children, if it has
    // any</td></tr>
    // </table>
    // <p>
    // Note, if you call <code>getTimingData()</code> directly from a 
    // +link{dsRequest.callback,DSRequest callback} or +link{RPCManager.sendQueue(),queue callback},
    // the timing data will be incomplete, because the queue at that point is incomplete (user 
    // callback processing is part of the processing that is being measured).  To get around
    // this, you can either move your <code>getTimingData()</code> call to thread-end by 
    // calling it on a 0ms delay:<pre>
    ...
    //    }); </pre>
    // <p>
    // Alternatively, you can move your <code>getTimingData()</code> call into a 
    // "processing complete" callback, and register it with the 
    // +link{addProcessingCompleteCallback()} API.  Processing complete callbacks fire when
    // the transaction is fully complete, including any user callbacks.
    // <p>
    // Also note, if you are using the +link{group:debugging,Developer Console}, timing data may
    // not be available synchronously from your application code, so you will have to implement
    // a callback function and pass it as the second parameter.  This is inconvenient, but it is
    // a browser restriction that we have no control over.
    // <p> 
    // If you call <code>getTimingData()</code> from your queue callback on a 0ms delay, or 
    // using a processing complete callback, as described above, the synchronous call is likely
    // to work.  If you introduce any kind of delay, or call <code>getTimingData()</code> in a 
    // way that is not synchronized with the transaction - for example, periodically on a timer,
    // or in response to some user action - the synchronous call is likely to return an error 
    // message stating that the timing data is unavailable synchronously, and you should 
    // provide a callback.  Again, all of this applies only if you have the SmartClient 
    // Developer Console open (and it applies equally to 
    // +link{RPCManager.getFormattedTimingData(),getFormattedTimingData()}).
    // <p>
    // Here is an example of how to provide a callback to the function:<pre>
    ...
    // </pre>
    // @param transactionNum (integer) The number of the transaction to return timing data for
    // @param [callback] (TimingDataCallback) A callback to invoke, passing the timing data
    //                                        object tree.  This parameter is optional, but
    //                                        it is necessary if timing data cannot be 
    //                                        obtained synchronously, as described above
    // @param [operationNo] (integer) Optionally, the index of a specific operation in the 
    //                                overall transaction. Only pass this parameter if you want
    //                                to limit the returned data to a single request
    // @param [includeClient] (boolean) Whether to include client-side timing data.  Optional
    ...
    // @param [includeServer] (boolean) Whether to include server-side timing data.  Optional, 
    //                                  defaults to true
    // @return (Object) the tree of timing data, as described in the above documentation
    // @see RPCManager.getFormattedTimingData()
    // @visibility internal in 13.0
    //<
    getTimingData : function(transactionNum, callback, operationNo, includeClient, includeServer) {

        var transaction;
        if (isc.isAn.Object(transactionNum)) {
            transaction = transactionNum;
        } else {
            if (isc.RPCTracker) {
                transaction = isc.RPCTracker.getTransactionRecord(transactionNum);
            } else if (isc.debugTarget && !this._transactions.find({transactionNum: transactionNum})) {
                
                transaction = isc.debugTarget.call("getTransactionRecord", [transactionNum], function(retval) {
                    isc.RPCManager.getTimingData(retval, callback, operationNo, includeClient, includeServer);
                });
                return "Timing data is not available synchronously because this transaction has " +
                            "been moved to the debugMaster.  " + (callback
                                ? "Timing data will be provided to your callback function"
                                : "Please implement a callback function to receive the timing data")
            }
            if (!transaction) {
                transaction = this.convertToTransactionRecord(
                                    this._transactions.find({transactionNum: transactionNum}));
            }
        }
    
        if (!transaction) {  
            return {name: "Transaction " + transactionNum + " not found"};
        }

        // If the transaction had been pushed to the debugMaster, and it only contained a 
        // single operation, the transaction-level data is dropped and "transaction" is 
        // actually the first (and only) "rpc" at this point.  Fortunately, the transaction
        // record is recorded on each operation, so we can recover it
        if (transaction.transactionRecord) {
            transaction = transaction.transactionRecord;
        }

        if (!transaction.rpcs || transaction.rpcs.length == 0) {  
            return {name: "Transaction " + transactionNum + " has no operations"};
        }
        ...
        var minOp = 0, 
            maxOp = transaction.rpcs.length;

        if (includeClient === undef) includeClient = true;
        if (includeServer === undef) includeServer = true;
        
        if (operationNo == null) {
            // User clicked a transaction record, so show the full list of timings
            var startTime = transaction.clientTiming ? transaction.clientTiming[0].start : transaction.sendTime;
            if (startTime == null && transaction.timeSent != null) startTime = transaction.timeSent.getTime();

            root = {
                timeout: transaction.timeout,
                name: "Complete client-server roundtrip",
                start: startTime,
                children: [],
                end: transaction.responseClientTiming ? 
                        transaction.responseClientTiming[transaction.responseClientTiming.length-1].end : 
                        transaction.responseReceived.getTime()
            }

            if (includeClient && transaction.clientTiming && transaction.clientTiming.length > 0) {
                var clientNode = {
                    timeout: transaction.timeout,
                    name: "Client processing (pre-server)",
                    start: transaction.clientTiming[0].start,
                    children: [],
                    end: transaction.clientTiming[transaction.clientTiming.length-1].end
                }
                root.children.add(clientNode);

                // The last clientTiming is "Marshall and send", and it applies at 
                // transaction level, not per-request.  So pull it out of the individual 
                // requests and just show it once against the queue
                var sendTiming = this.cleanDup(transaction.clientTiming[0].children[transaction.clientTiming[0].children.length-1]);

                for (var i = minOp; i < maxOp; i++) {

                    var ct, name = "Operation " + i + " client processing";
                    if (transaction.clientTiming[i] == null) {
                        isc.logWarn("Some clientTiming entries are missing.  For a multi-request " +
                                    "queue, your code should call getTimingData() from the " +
                                    "queue's callback method, *not* from an individual " +
                                    "request's callback");
                        ct = {
                            children: []
                        }
                        name += " (data missing, see log)";
                    } else {
                        ct = this.cleanDup(isc.addProperties({}, transaction.clientTiming[i]));
                        ct.children.removeAt(ct.children.length-1);
                    }
                    clientNode.children.add({
                        timeout: transaction.rpcs[i].timeout,
                        name: name,
                        start: ct.children.length > 0 ? ct.children[0].start  : 0,
                        children: ct.children,
                        end:  ct.children.length > 0 ? ct.children[ct.children.length-1].end : 0
                    });
                }

                clientNode.children.add(sendTiming);

            }

            if (includeServer && transaction.serverTiming && transaction.serverTiming.length > 0) 
            {
                var serverStart = transaction.clientTiming && transaction.clientTiming.length > 0 ? 
                                    transaction.clientTiming[transaction.clientTiming.length-1].end :
                                    root.start;
                var serverEnd = transaction.responseClientTiming && transaction.responseClientTiming.length > 0 ?
                                    transaction.responseClientTiming[0].start :
                                    root.end;
                ...
                var serverTrip = {
                    ...
                    start: serverStart,
                    children: [],
                    end: serverEnd == undef ? transaction.responseReceived : serverEnd
                };

                root.children.add(serverTrip);

                // Pull the RPCManager-level timings up to transaction level
                var rpcConstructTiming = this.cleanDup(transaction.serverTiming[0].children[0]);
                var rpcExecuteTiming = this.cleanDup(transaction.serverTiming[0].children[1]);
                var serializeTiming = this.cleanDup(transaction.serverTiming[0].children[transaction.serverTiming[0].children.length-1]);
                serverTrip.children.add(rpcConstructTiming);
                serverTrip.children.add(rpcExecuteTiming);

                //isc.logWarn("Server Timing just before processing: " + isc.Comm.serialize(transaction.serverTiming));


                var opLevelChildren = [];

                for (var i = minOp; i < maxOp; i++) {
                    var opExecuteTiming = this.cleanDup(transaction.serverTiming[i].children[1]);

                    opLevelChildren.add({
                        timeout: transaction.rpcs[i].timeout,
                        name: "Operation " + i + " server processing",
                        start: opExecuteTiming.children[0].start,
                        children: opExecuteTiming.children,
                        end: opExecuteTiming.children[opExecuteTiming.children.length-1].end
                    });
                }
                rpcExecuteTiming.children = opLevelChildren;
                serverTrip.children.add(serializeTiming);

                var qstart = transaction.serverTiming[0] ? transaction.serverTiming[0].start : -1,
                    last = transaction.serverTiming.length-1,
                    qend   = transaction.serverTiming[last] ? transaction.serverTiming[last].end : -1;
                var nodes = this.getNetworkTimeNodes(serverStart, transaction.responseReceived.getTime(), 
                                        qstart, qend);
                serverTrip.children.addAt(nodes.pre, 0);
                serverTrip.children.add(nodes.post);
            }

            if (includeClient && transaction.responseClientTiming && transaction.responseClientTiming.length > 0) {
                var clientNode = {
                    timeout: transaction.timeout,
                    name: "Client processing (post-server)",
                    start: transaction.responseClientTiming[0].start,
                    children: [],
                    end: transaction.responseClientTiming[transaction.responseClientTiming.length-1].end
                }
                root.children.add(clientNode);

                // The first responseClientTiming relates to parsing the response text, and the
                // last one relates to time spent in the transaction-level user callback (if 
                // there are three child nodes - sometimes there are only two, relating to 
                // parsing the response text and performOperationReply).  Both of these are 
                // transaction-level, so pull them out
                var parseTiming = this.cleanDup(transaction.responseClientTiming[0].children[0]);
                var callbackTiming;
                if (transaction.responseClientTiming[0].children.length == 3) {
                    callbackTiming = this.cleanDup(transaction.responseClientTiming[0].children[
                                                        transaction.responseClientTiming[0].children.length-1]);
                } 
                clientNode.children.add(parseTiming);


                for (var i = minOp; i < maxOp; i++) {
                    var ct, name = "Operation " + i + " client processing";
                    if (transaction.responseClientTiming[i] == null) {
                        isc.logWarn("Some clientTiming entries are missing.  For a multi-request " +
                                    "queue, your code should call getTimingData() from the " +
                                    "queue's callback method, *not* from an individual " +
                                    "request's callback");
                        ...
                        ct = this.cleanDup(transaction.responseClientTiming[i]);
                        if (callbackTiming) {
                            ct.children.removeAt(ct.children.length-1);
                        }
                        ct.children.removeAt(0);
                    }
                    clientNode.children.add({
                        timeout: transaction.rpcs[i].timeout,
                        name: name,
                        // Can't rely on start and end on this node because it is trx-level
                        // rather than per-operation
                        start: ct.children[0].start,
                        children: ct.children,
                        end: ct.children[ct.children.length-1].end
                    });
                }
                if (callbackTiming) {
                    clientNode.children.add(callbackTiming);
                }
            }

        } else {
            var overallTrx = transaction;
            transaction = transaction.rpcs[operationNo];
            
            root = {
                timeout: transaction.timeout,
                name: "Complete client-server roundtrip",
                start: overallTrx.timeSent.getTime(),
                children: [],
                end: overallTrx.responseReceived.getTime()
            }

            if (includeClient && transaction.clientTiming) {
                var node = this.cleanDup(transaction.clientTiming, {name: "Client processing (pre-server)"});
                root.children.add(node);
                root.start = transaction.clientTiming.start;
            }

            if (includeServer && transaction.serverTiming) {
                var serverTrip = {
                    timeout: transaction.timeout,
                    name: "Server roundtrip",
                    start: transaction.clientTiming ? transaction.clientTiming.end : root.start,
                    children: [],
                    end: transaction.responseReceived.getTime()
                }
                
                root.children.add(serverTrip);

                serverTrip.children.add(this.cleanDup(transaction.serverTiming));
                if (transaction.serverTiming.start && transaction.serverTiming.end) {
                    if (serverTrip.start && serverTrip.end) {
                        var nodes = this.getNetworkTimeNodes(serverTrip.start, serverTrip.end, 
                                            transaction.serverTiming.start, transaction.serverTiming.end);
                        serverTrip.children.addAt(nodes.pre, 0);
                        serverTrip.children.add(nodes.post);
                    }
                }
            ...
            if (includeClient && overallTrx.responseClientTiming) {
                var clientNode = this.cleanDup(overallTrx.responseClientTiming[operationNo], 
                                                        {name: "Client processing (post-server)"});
                root.children.add(clientNode);
                root.end = clientNode.end;
            }

        }
        
        var returnValue = this.populateElapsed(root);
        returnValue = isc.Tree.create({ 
            root: returnValue,
            modelType: "children"
        });
        if (callback) {
    ...
    cleanDup : function(obj, overrideProps, subcall) {
        if (!subcall) {
            this._cleanDupAlreadySeen = [];
        }
        if (isc.isAn.Array(obj)) {
            var targetArray = [];
            for (var i = 0; i < obj.length; i++) {
                targetArray[i] = this.cleanDup(obj[i], null, true);
            }
            return targetArray;
        }
        if (isc.isAn.Object(obj)) {
            var targetObj = {};
            for (var key in obj) {
                if (!key.startsWith("_")) {
                    var value = (overrideProps ? overrideProps[key] : null) || obj[key];
                    if (this.cleanDupAlreadySeen(value)) {
                        value = "*DUPLICATE*";
                    } else {
                        if (isc.isAn.Array(value) || isc.isAn.Object(value)) {
                            // overrideProps only applies to the top-level object
                            value = this.cleanDup(value, null, true);
                            this._cleanDupAlreadySeen.add(value);
                        }
                    }
                    targetObj[key] = value;
                }
    ...
    cleanDupAlreadySeen : function(value) {
        if (!this._cleanDupAlreadySeen) return false;
        for (var i = 0; i < this._cleanDupAlreadySeen.length; i++) {
            if (this._cleanDupAlreadySeen[i] === value) return true;
        }
        return false;
    },

    populateElapsed : function(node) {
        ...
        for (var i = 0; i < nextLevel.length; i++) {
            this.populateElapsed(nextLevel[i]);
        }
        return node;
    },

    convertToTransactionRecord : function(transaction) {

        // We end up in this flow with a completely empty transaction (one with no operations)
        // if the Dev Console is open at the time of transaction creation, because the 
        // transaction's changed() method invokes pushRPCUpdate() if we have a debugTarget.  
        // So just skip this processing in that case
        if (!transaction || !transaction.operations || transaction.operations.length == 0) {
            return null;
        }

        var clientOnly = !!transaction.allClientOnly;
        ...
        var rpcs = [];
        var clientTiming = [], serverTiming = [], responseClientTiming = [];
        for (var i = 0; i < transaction.operations.length; i++) {
            var rpcRequest = transaction.operations[i];
            ...
            if (rpcResponse) {
                ...
                if (rpcRequest.clientTiming) {
                    clientTiming[i] = isc.clone(rpcRequest.clientTiming);
                    rpcs[i].clientTiming = clientTiming[i];
                }
                if (rpcResponse.clientTiming) {
                    responseClientTiming[i] = isc.clone(rpcResponse.clientTiming);
                    rpcs[i].responseClientTiming = clientTiming[i];
                }
                if (!clientOnly && rpcResponse.timing) {
                    serverTiming[i] = isc.clone(rpcResponse.timing);
                    rpcs[i].serverTiming = serverTiming[i];
                }

                rpcs[i].transactionRecord = transactionRecord;
            }
            
            if (transaction.operations.length > 1) rpcs[i]._queueIndex = i;
        }    
        transactionRecord.rpcs = rpcs;
        if (clientTiming.length > 0) {
            transactionRecord.clientTiming = clientTiming;
        }
        if (responseClientTiming.length > 0) {
            transactionRecord.responseClientTiming = responseClientTiming;
        }
        if (!clientOnly && serverTiming.length > 0) {
            transactionRecord.serverTiming = serverTiming;
        }

        transactionRecord.timeSent = rpcs[0].timeSent;
        transactionRecord.responseReceived = rpcs[rpcs.length-1].responseReceived;
    ...
    rpcToRecord : function (transaction, rpcRequest, rpcResponse) {                
        ...
        if (rpcResponse) {
            ...
            resp.responseClientTiming = resp.clientTiming;
    ...
    getNetworkTimeNodes : function(clientStart, clientEnd, serverStart, serverEnd) {
        var nodes = {};
        var totalTime = clientEnd - clientStart,
            serverTime = serverEnd - serverStart,
            delta = totalTime - serverTime;
        if (delta % 2) delta += 1;
        delta /= 2;
        nodes.pre = {
            name: "Network time (inferred)",
            start: clientStart,
            end: clientStart + delta
        };
        nodes.post = {
            name: "Network time (inferred)",
            start: clientStart + delta + serverTime,
            end: clientStart + delta + serverTime + delta
        };
        return nodes;
    },
	...
	performOperationReply : function (request, response) {
        ...
        if (this.logIsInfoEnabled()) {
            if (response.downloadResult) {
                this.logInfo("processing rpcResponse to XHR download request");
            } else {
        ...
        var errorResponse = response.isStructured && 
                            (!results || (results.status < 0) ||
                            (results.status == null && response.status < 0)); 

		if (errorResponse && request.willHandleError != true) {
    	    return this._handleError(response, request);
        }
        
        // If downloadResult is true, show the file download dialog, allowing the user to
        // download the response payload into a file
        // Skip this if we got an error response - in this case
        // the response is likely not a valid data object to download, and 'willHandleError' 
        // must be true (so we'll allow the error handling callback to fire)
        if (!errorResponse && request.downloadResult) {
            this.downloadXmlHttpResponseData(results, response, request);
            // Suppress callbacks - this is done to maintain consistency with hiddenFrame 
            // downloads
            return;
        }
    ...
    // Helper method to download the contents of an XHR reponse to a file
    // used when request.downloadResult is true and request is explicitly set to "xmlHttpRequest"
    
    downloadXmlHttpResponseData : function (data, response, request) {

        // file name for download may be explicitly specified on the request
        var fileName = request.download_filename || request.exportFilename,
            type;    

        // Extract details from the XHR headers about the data, if present
        if (response.httpHeaders) {
            // If we don't already have a filename, extract it from content-disposition,
            // if present
            if (fileName == null) {
                var disposition = response.httpHeaders["content-disposition"],
                    dispFilenameStart = isc.isA.String(disposition) ? disposition.search(new RegExp("filename", "i")) : -1;

                if (dispFilenameStart > 0) {
                    dispFilenameStart = disposition.indexOf("=", dispFilenameStart) +1; // may be filename=... or filename*=...
                    var dispFilenameEnd = disposition.indexOf(";", dispFilenameStart); // properties are separated with semicolons
                    if (dispFilenameEnd == -1) dispFilenameEnd = disposition.length;

                    fileName = disposition.substring(dispFilenameStart, dispFilenameEnd);
            ...
            // mimetype - we can specify this on the link itself if present in the headers
            var type = response.httpHeaders["content-type"];
        }

        // this creates a new element with a data URL and simulates a click on it
    ...
    fireReplyCallbacks : function (request, response) {
        ...
        var aemSuspended = false;
        if (isc.Class._aemActive && request._isAEMExcludedTransaction) {
            isc.Class.suspendAEM();
            aemSuspended = true;
        }
        ...
        if (aemSuspended) isc.Class.unsuspendAEM();
	...
	runDefaultErrorHandling : function (response, request, errorFormatter) {
		...
		if (isc.isA.String(response.data)) {
            ...
            if (message != null) {
                ...
                } else if (codeName == "CRITERIA_REQUIRED_ERROR" || codeName == "REQUIRED_CRITERIA_MISSING") {
    ...
    // @param httpResponseText (String) The raw HTTP Response text
    ...
    //> @classAttr RPCManager.reloginCommFailureMessage (HTMLString : "The server is unavailable right now, please try again in a while" : IRW)
    // Message that will be reported in the relogin window if relogin fails due to communication
    // problems.
    // @visibility external
    // @group relogin
    // @group i18nMessages
    //<
    reloginCommFailureMessage: 
        "The server is unavailable right now, please try again in a while",
...
// <ul><li>It includes the +link{RPCManager.loginRequiredMarker}. Responses to standard 
//      +link{RPCRequest.transport,xmlHttpRequest transport} RPC requests will be scanned for this marker
//      and kick off the +link{RPCManager.loginRequired(),loginRequired flow} automatically.</li>
//    <li>For +link{RPCRequest.transport,hiddenFrame transport} request, it also includes some 
//      JavaScript to explicitly notify the RPCManager in the SmartClient app running in the
//      parent frame or opener window that login is required.</li>
// </ul>
// <P>
...
//<
    ...
    handleLoginRequired : function (transactionNum) {
        ...
        if (this._iscReloginWindow && !this._iscReloginWindow.closed && this._iscReloginWindow == transactionNum) 
            return;
    ...
    //> @classAattr RPCManager.supportsPostMessageRelogin (boolean : false : IRWA)
    // Do we allow the login status to be set by issuing a native 
    // +externalLink{https://developer.mozilla.org/en-US/docs/Web/API/Window/postMessage,postMessage()} 
    // notification to this window from another frame?
    // <P>
    // The standard marker HTML snippets available in the SDK under
    // <smartclient>smartclientSDK/isomorphic/login/loginRequiredMarker.html, 
    // smartclientSDK/isomorphic/login/loginSuccessMarker.html, and
    // smartclientSDK/isomorphic/login/maxLoginAttemptsExeededMarker.html</smartclient>
    // <smartgwt>docs/loginRequiredMarker.html,  
    // docs/loginSuccessMarker.html, and
    // docs/maxLoginAttemptsExeededMarker.html</smartgwt> make use of this
    // mechanism.
    // <P>
    // @see RPCManager.processLoginStatusMessage()
    //
    //<
    supportsPostMessageRelogin:false,

    //> @classMethod setPostMessageReloginEnabled()
    // Setter for +link{supportsPostMessageRelogin}
    // @param enabled (boolean) Whether we should support setting login status from another frame via postMessage
    //< 
    setPostMessageReloginEnabled : function (enabled) {
        if (this.supportsPostMessageRelogin == enabled) return;

        this.supportsPostMessageRelogin = enabled;
        if (enabled) {
            this.setupReloginListener();
        } else {
            this.clearReloginListener();
        }

    },
    setupReloginListener : function () {
        this._reloginEventHandler = function (event) {
            if (!isc.RPCManager.supportsPostMessageRelogin) {
                
                return;
            }
            isc.RPCManager.processLoginStatusMessage(event);
        }
        window.addEventListener(
            "message", 
            this._reloginEventHandler,
            false
        )
    },
    clearReloginListener : function () {
        if (this._reloginEventHandler != null) {
            window.removeEventListener("message", this._reloginEventHandler, false);
            delete this._reloginEventHandler;
        }
    },

    //> @classAttr RPCManager.postMessageReloginOriginPolicy (String | Array of String : "*" : IRWA)
    // If +link{supportsPostMessageRelogin} is true, this property may be set to
    // restrict the origin from which we will process relogin requests.
    // Valid settings are
    // <ul><li><code>"*"</code> - no restrictions on message source origin</li>
    //      <li><code>"same"</code> - origin of message source must match <code>window.origin</code></li>
    //      <li>Any array of allowed origins</li></ul>
    // @see RPCManager.processLoginStatusMessage()
    //<
    postMessageReloginOriginPolicy:"*",

    //> @classMethod RPCManager.processLoginStatusMessage()
    // If +link{supportsPostMessageRelogin} is true, this method will be invoked when we receive 
    // +externalLinkhttps://developer.mozilla.org/en-US/docs/Web/API/Window/postMessage,post message notifications}.
    // <P>
    // If <code>event.data</code> is one of the following values it will take action:
    // <ul><li><code>"isc_loginRequired"</code>: Invokes the +link{loginRequired()} flow</li>
    //     <li><code>"isc_loginSuccess"</code>: Processes a successful relogin attempt</li>
    //     <li><code>"isc_maxLoginAttemptsExceeded"</code>: Invokes the +link{maxLoginAttemptsExceeded()} flow</li>
    // </ul>
    // <P>
    // If the source of the message event was not a +link{RPCRequest.transport,hiddenFrame} or 
    // a relogin window generated automatically by the framework, the +link{postMessageReloginOriginPolicy} will
    // be consulted, allowing developers to only allow login status messages from specific origin(s).
    //
    // @param event (DOMEvent) The native +link{https://developer.mozilla.org/en-US/docs/Web/API/MessageEvent,message event}.
    //<
    
    processLoginStatusMessage : function (event) {

        var loginStatus = event.data,
            loginRequired = loginStatus == "isc_loginRequired",
            loginSuccess = loginStatus == "isc_loginSuccess",
            maxLoginAttemptsExceeded = loginStatus == "isc_maxLoginAttemptsExceeded";
        
        // Ignore anything that isn't a simple login status notification
        if (!loginRequired && loginSuccess && maxLoginAttemptsExceeded) return;

        // Did the message come from a hidden frame or our built-in log-in popout?
        var reloginWindow = this._iscReloginWindow;
        if (reloginWindow && reloginWindow.closed) reloginWindow = null;
        var recognizedSource = event.source == window || event.source == reloginWindow;
        if (!recognizedSource) {
            var hiddenFrames = isc.HiddenFrame._hiddenFrames;
            for (var i = 0; i < hiddenFrames.length; i++) {
                if (event.source == hiddenFrames[i].getHandle()) {
        ...
        // If we have an originPolicy - did the message come from the appropriate origin?
        if (!recognizedSource) {
            var originPolicy = this.postMessageReloginOriginPolicy;
            if (originPolicy != "*" && originPolicy != null) {
                if (!isc.isAn.Array(originPolicy)) originPolicy = [originPolicy];
                var allowedOrigin = false;
                for (var i = 0; i < originPolicy.length; i++) {
                    var origin = originPolicy[i];
                    if (origin == "same") origin = window.origin;
                ...
                if (!allowedOrigin) {
                    this.logWarn("RPCManager - ignoring attempt to set login status via postMessage() from origin:" + event.origin + 
                        ". Configure the RPCManager.postMessageReloginOriginPolicy if you want to allow login status to be set from this origin.");
                    return;
                }
            }
        }

        // Pass the window to handleLoginRequired() - this will look up the
        // appropriate transaction
        if (loginRequired) {
            isc.RPCManager.handleLoginRequired(event.source);
        } else if (loginSuccess) {
            isc.RPCManager.handleLoginSuccess(event.source);
        } else if (maxLoginAttemptsExceeded) {
            isc.RPCManager.handleMaxLoginAttemptsExceeded(event.source);
        }
    },    
...
//<
    ...
    handleLoginSuccess : function (transactionNum) {
        ...
        if (this._iscReloginWindow) {
            this._iscReloginWindow.close();
            delete this._iscReloginWindow;
        }
    ...
    // <P>
    // Also, you can set the filename of the export via +link{dsRequest.exportFilename}. For example:
    // <smartclient>
    // <pre>
    // var settings = {
    ...
    // This is the bundle of settings that can be passed to +link{RPCManager.loadScreen()} as
    // the "settings" argument.  Some settings can also be passed as separate arguments; if
    // these are present both as separate arguments and in settings, loadScreen() will use the
    // value from the settings.
    // <P>
    // There is no need to instantiate a <code>LoadScreenSettings</code> instance.  Just pass a
    // normal JavaScript object with the desired properties.
    //
    // @visibility external
    // @treeLocation Client Reference/Tools
    //<
    
    //> @attr loadScreenSettings.locale (String : null : IRW)
    // The name of a locale to use for resolving i18n tags in the component XML of the screen.
    // @visibility external
    //<

    //> @attr loadScreenSettings.requestProperties (RPCRequest Properties : null : IRW)
    // Specifies optional +link{RPCRequest} properties for the request.
    // @visibility external
    //<

    //> @attr loadScreenSettings.missingDSIsNotFatal (Boolean : null : IRW)
    // If true, server logic does not crash out if it cannot load a DataSource specified in the
    ...
    //> @attr loadScreenSettings.willHandleError (Boolean : null : IR)
    // Whether to call the provided +link{Callbacks.LoadScreenCallback,callback} even if an
    // error was encountered trying to load the screen (or it was simply not found), so that
    // you can run your own error handling.  In this case, the screen and "suppressed globals"
    // will be reported as null.
    // <P>
    // Note that for backward compatibility, the default of null means that the callback will
    // still happen if the screen is not found, but not for a response with a server error.
    // To have all such cases handled automatically, set this property explicitly false.
    // @visibility external
    //<

    //> @attr loadScreenSettings.clobberDataSources (Boolean : false : IRW)
    // Should DataSources referenced by the screen clobber existing, globally-bound DataSources
    // on the client when the screen is loaded?  The default of false means that any
    // DataSources defined in the screen will be discarded if they collide with existing,
    // globally-bound DataSources.
    // <p>
    // Note that here we consider a DataSource to be "globally bound" if it can be retrieved by
    // ID using the method +link{DataSource.get()}, regardless of whether it's actually bound
    // to the browser <code>window</code> object.
    // @visibility external
    //<

    //> @attr loadScreenSettings.omitDataSources (Array of Identifier : varies : IRW)
    // DataSource IDs in the screen to skip and not load when the screen is loaded.  It is
    // assumed that these IDs represent DataSources that are already globally-bound on the
    // client.
    // <P>
    // The special value of "*" can be specified for this property to indicate that <b>all</b>
    // DataSources should be omitted.
    // <P>
    // Note that, unless +link{omitLoadedDataSources} is false, all loaded DataSources will by
    // default be added to whatever value you provide (making that also the default for this
    // property).
    // @visibility external
    //<

    //> @attr loadScreenSettings.omitLoadedDataSources (boolean : true : IRW)
    // Whether to implicitly add all DataSources currently loaded on the client to
    // +link{omitDataSources} in +link{RPCManager.loadScreen()}.  Setting this false would only
    // make sense in connection with setting +link{clobberDataSources} true, and would create
    // more potential work for the server since many more DataSources could be output.
    // <P>
    // Note that here we consider the "loaded DataSources" to be those that are registered
    // with the DataSource module (i.e. available by ID via +link{DataSource.get()}),
    // regardless of whether they're actually bound to the browser <code>window</code> object.
    // @visibility external
    //<

    //> @attr loadScreenSettings.suppressAutoDraw (Boolean: true : IR)
    // If true, prevents any screen from being drawn when it's loaded, even if
    // there's an explicit +link{canvas.autoDraw}:true setting on it.
    // @visibility external
    //<    

    //> @attr loadScreenSettings.cacheScreen (Boolean: false : IR)
    // Caches screen much like +link{RPCManager.cacheScreens()}, so
    // +link{RPCManager.isScreenCached()} reports true.
    // @visibility external
    //<    

    //> @attr loadScreenSettings.verifyComponents (Map<String, String> : null : IR)
    ...
    // with a warning dialog by setting +link{loadScreenSettings.verifyAsError} or
    // +link{RPCManager.verifyAsError}.
    // @see RPCManager.loadScreen
    // @visibility external
    //<

    //> @attr loadScreenSettings.verifyAsError (boolean : null : IRW)
    // Enable +link{RPCManager.verifyAsError, verifyAsError} behavior only for
    // requests using these settings.
    // @see createScreenSettings.verifyAsError
    // @see loadProjectSettings.verifyAsError
    // @visibility external
    //<

    //> @classAttr RPCManager.verifyAsError (boolean : null : IRW)
    // When true, any problems found by either
    // +link{loadProjectSettings.verifyComponents, verifyComponents} or
    // +link{loadProjectSettings.verifyDataSources, verifyDataSources} are shown together in a
    // dialog presented to the end user.  Provides default values for
    // +link{loadProjectSettings.verifyAsError}, +link{loadScreenSettings.verifyAsError},
    // and +link{createScreenSettings.verifyAsError}.
    //
    // @see createScreenSettings.verifyAsError
    // @see loadScreenSettings.verifyAsError
    // @see loadProjectSettings.verifyAsError
    // @visibility external
    //<
    ...
    // is true of any screens created in Reify).  
    // <p>
    ...
    // @param [settings] (Array of String | LoadScreenSettings) widgets to allow to take their
    //                                                          global IDs, or load settings
    ...
    // @visibility external
    //<
    loadScreen : function (screenName, callback, settings, locale, requestProperties,
                           missingDSIsNotFatal) 
    {
        if (!screenName) {
        ...
        // settings can either be loadScreenSettings, or globals (for backcompat)
        var globals;
        if (!isc.isAn.Array(settings) && isc.isAn.Object(settings)) {
            globals = settings.globals || [];
            // populate legacy named arguments from settings if required
            if (settings.locale)            locale            = settings.locale;
            if (settings.requestProperties) requestProperties = settings.requestProperties;
            if (settings.missingDSIsNotFatal != null) {
                missingDSIsNotFatal = settings.missingDSIsNotFatal;
            }
        } else { // settings is actually the legacy "globals" argument
        ...
        var requestParams = {};
        requestParams.screenName = screenName.join(",");
        if (missingDSIsNotFatal) {
            requestParams.missingDSIsNotFatal = true;
        }
        if (locale) requestParams.locale = locale;
        if (requestProperties && requestProperties.mockMode != null) {
            isc.addProperties(requestParams, {mockMode: requestProperties.mockMode});
        }

        // allow caching of the loaded screens
        var cacheScreen = settings.cacheScreen;
        if (cacheScreen) requestParams.structuredResponse = true;

        // build param for omitDataSources, the DS to ignore
        this._addOmittedDataSources(settings, requestParams);

        var _this = this;
        var requestCallback = function (rpcResponse, data, rpcRequest) {
            if (cacheScreen) {
                var screens = _this._cacheScreensFromData(data);
                data = _this._buildCodeFromScreens(screens);
            }
            if (settings.willHandleError) {
                if (rpcResponse.status < 0 || !data) {
                    _this.fireCallback(callback, "screen,rpcResponse,suppressedGlobals",
                                       [null, rpcResponse, null]);
                    return;
                }
            } else if (settings.willHandleError == false && !data) {
                isc.RPCManager.reportError("Load failed - Screen \"" + screenName +
                                           "\" not found on the server");
                return;
            }
            _this._makeScreen(rpcResponse, data, rpcRequest,
                function (screen, rpcResponse, suppressedGlobals) {
                    if (screen && settings.verifyComponents) {
                        var report = _this.verifyComponents(screen, settings.verifyComponents);
                        _this.logVerificationsResult(report, settings, rpcResponse);
                    }

                    _this.fireCallback(callback, "screen,rpcResponse,suppressedGlobals",
                                       [screen, rpcResponse, suppressedGlobals]);
                }, globals, { // common create/load properties
                    clobberDataSources: settings.clobberDataSources,
                    suppressAutoDraw: settings.suppressAutoDraw != false
                }
            );
        };

        return this._loadComponentXML(requestParams, requestCallback, settings,
                                      requestProperties);
    },

    // Issue a request against the screenLoaderServlet to load componentXML
    // Used to load screens but also Processes, Tours. 
    _loadComponentXML : function (requestParams, callback, settings, requestProperties) {

        var request = {};
        if (requestProperties) isc.addProperties(request, requestProperties);
        if (request.params == null) request.params = {};
        isc.addProperties(request.params, requestParams);

        // if willHandleError in the request is not already set, default it from the settings
        if (request.willHandleError == null && settings && settings.willHandleError != null) {
            request.willHandleError = settings.willHandleError;
        }
    ...
    // <P>
    // There is no need to instantiate an LoadProjectSettings instance.  Just pass a normal
    ...
    // argument to +link{rpcManager.loadProject}.
    // @visibility external
    //<
    
    //> @attr loadProjectSettings.screenNames (String : null : IRW)
    // A comma-separated string containing the names of screens within the project that should
    // be loaded.  A null value causes all screens to be loaded.
    // <p>
    // This setting only affects the first project specified in the <code>projectNames</code>
    // argument to +link{rpcManager.loadProject}.
    // @visibility external
    ...
    //> @attr loadProjectSettings.drawFirstScreen (boolean : false : IRW)
    // Determines whether the +link{loadProjectSettings.currentScreenName} screen is drawn
    // after all screens have been loaded.  If not drawn, the screen will not be created.
    // <p>
    // This setting only affects the first project specified in the <code>projectNames</code>
    // argument to +link{rpcManager.loadProject}.
    // @see project.createScreen()
    ...
    // The name of a locale to use for resolving i18n tags in the component XML of the screen.
    // The default value of null omits locale loading, which effectively means the framework
    ...
    //> @attr loadProjectSettings.allowPlaceholders (Boolean : null : IR)
    // Should +link{Placeholder,Placeholders} in loaded screens be rendered as placeholders?
    // If property is not set actual components are created instead of the Placeholders.
    // @visibility external
    //<

    //> @attr loadProjectSettings.dataContext (DataContext : null : IR)
    // +link{DataContext} that will be provided to the top-level component as
    // +link{canvas.dataContext,dataContext} in each screen cached for the project.
    // <P>
    ...
    //> @attr loadProjectSettings.willHandleError (boolean : false : IR)
    // Whether to call the provided +link{Callbacks.LoadProjectCallback,callback} even if an
    // error was encountered trying to load the project, so that you can run your own error
    // handling.  In this case, the list of projects loaded by the callback will be reported as
    // empty.
    // <P>
    // If true, the framework won't log any messages specifically reporting the failure to
    // load the requested projects, but depending on the situation, the browser itself may
    // report errors from the servlet request in the console.
    // @see RPCManager.getLoadProjectErrorStatus()
    // @see RPCManager.getLoadProjectErrorMessage()
    // @visibility external
    //<

    //> @attr loadProjectSettings.requestProperties (RPCRequest Properties : null : IRA)
    // Allows customizing the projectLoader servlet request properties.  Properties that
    // conflict with proper operation of the servlet will be overridden by
    // +link{RPCManager.loadProject()}.
    // @visibility external
    //<

    //> @attr loadProjectSettings.timeout (int : null : IRA)
    // Sets the timeout for the projectLoader servlet request.  This is a convenience property
    // so that +link{requestProperties} need not be set.  If unset, the timeout is determined
    // by +link{RPCManager.defaultTimeout}.
    // @visibility external
    //<

    //> @attr loadProjectSettings.clobberDataSources (Boolean : false : IRW)
    // Should DataSources referenced by the +link{drawFirstScreen,first screen} clobber
    // existing, globally-bound DataSources on the client if the screen is created?  The
    // default of false means that any DataSources defined in the screen will be discarded if
    // they collide with existing, globally-bound DataSources.
    // <p>
    // Here we consider a DataSource to be "globally bound" if it can be retrieved by
    // ID using the method +link{DataSource.get()}, regardless of whether it's actually bound
    // to the browser <code>window</code> object.
    // <P>
    // Note that this setting only has an impact if +link{drawFirstScreen} is true.
    // @visibility external
    //<

    //> @attr loadProjectSettings.omitDataSources (Array of Identifier : varies : IR)
    // DataSource IDs in the project to skip and not load when the project is loaded.  It is
    // assumed that these IDs represent DataSources that are already globally-bound on the
    // client.
    // <P>
    // The special value of "*" can be specified for this property to indicate that <b>all</b>
    ...
    //> @attr loadProjectSettings.omitLoadedDataSources (boolean : true : IRW)
    // Whether to implicitly add all DataSources currently loaded on the client to
    // +link{omitDataSources} in +link{RPCManager.loadProject()}.  Setting this false would
    // only make sense in connection with setting +link{clobberDataSources} true, and would
    // create more potential work for the server since many more DataSources could be output.
    // <P>
    // Note that here we consider the "loaded DataSources" to be those that are registered
    // with the DataSource module (i.e. available by ID via +link{DataSource.get()}),
    // regardless of whether they're actually bound to the browser <code>window</code> object.
    ...
    //> @attr loadProjectSettings.verifyDataSources (Boolean : null : IR)
    // Enables DataSource verification, causing warnings
    // to be output about differences between DataSources loaded with a project vs those
    // that are already present in the page (if any are present).  Findings are always reported
    // to the console, and may also be presented to the user with a warning dialog by setting
    // +link{loadProjectSettings.verifyAsError}.  For discussion of which issues will be
    // reported and how, see +link{DataSource.verifyDataSourcePair()}.
    // <P>
    // Setting this property will default +link{omitLoadedDataSources} to false, and will cause
    // the special +link{omitDataSources} value of "*" to be ignored (but not other ID values).
    // It's important to keep in mind when using this property that loading a project
    // with all of its DataSources can be very slow, if there is a large amount of test data,
    // for example.
    // <P>
    // Note that this behavior can be enabled globally by setting
    // +link{isc.Reify.verifyDataSources} to true.  It often makes sense to set it there during
    // development, and turn off in production, since it slightly slows down DataSource
    // loading.
    // @see loadProjectSettings.verifyComponents
    // @see RPCManager.loadProject
    // @visibility external
    //<
    _shouldVerifyLoadProjectDS : function (settings) {
        if (settings.verifyDataSources != null) return settings.verifyDataSources;
        return isc.Reify && isc.Reify.verifyDataSources;
    },

    //> @attr loadProjectSettings.verifyAsError (boolean : null : IRW)
    // Enable +link{RPCManager.verifyAsError, verifyAsError} behavior only for
    // requests using these settings.
    // @see createScreenSettings.verifyAsError
    // @see loadProjectSettings.verifyAsError
    ...
    //> @type CustomizationEnv
    //
    // @value "designTime" Should apply when using the Reify Tool
    // @value "runTime"    Should apply when deploying a project
    // @value "both"       Should apply in both situations
    //<

    //> @attr loadProjectSettings.includeCustomizations (CustomizationEnv: null : IR)
    // Whether to include JS from the isc_reifyCustomizations DS in the project.
    //<

    //> @attr loadProjectSettings.verifyComponents (Map<String, String> : null : IR)
    // Enables verification that any screen +link{loadProjectSettings.drawFirstScreen, created}
    ...
    // to the console, and may also be presented to the user with a warning dialog by setting
    // +link{loadProjectSettings.verifyAsError} or +link{RPCManager.verifyAsError}.
    // @see loadProjectSettings.verifyDataSources
    // @see RPCManager.loadProject
    // @visibility external
    //<

    //> @method Callbacks.LoadProjectCallback
    // A +link{type:Callback} to evaluate after +link{RPCManager.loadProject()} completes.
    // <P>
    // If +link{loadProjectSettings.willHandleError} is set, the callback will
    // fire even if the requested projects could not be retrieved.  You can call
    // +link{RPCManager.getLoadProjectErrorStatus()} or
    // +link{RPCManager.getLoadProjectErrorMessage()} in this case for more information.
    //
    // @param project  (Project)          the first project loaded
    // @param projects (Array of Project) array of all projects loaded
    // @param rpcResponse (RPCResponse)   server response
    //
    // @visibility external
    //<
    ...
    // +link{rpcManager.projectLoaderURL}, and fires the given callback after the project has
    // been cached.  When a project is loaded, all of its DataSources and screens (except where
    // explicitly overridden by settings) are also cached in the project.
    // <p>
    // Loading of a project merely caches DataSources and screens.  No screens are created
    // automatically.  To create a screen, you must call +link{project.createScreen()} or
    // +link{project.createStartScreen()}, which will automatically create any DataSource
    // needed by the screen that's not already globally bound.  You can also manually
    // instantiate and globally bind (if not already bound) a screen DataSource without
    // creating the screen, by calling +link{project.getDataSource()}.
    // <p>
    // Note that any screen cached in a loaded project will also be bound in the global cache
    // if no screen of that name is present there.  You can create a screen from the global
    // cache using +link{createScreen()}.
    // <p>
    // When projects are cached, they're made available via +link{project.get()}.  To remove a
    // cached project, and release storage for its cached screens and DataSources, simply call
    ...
    loadProject : function (projectNames, callback, settings) {
        if (!projectNames) {
            this.logWarn("loadProject(): no project name passed in");
            this.fireCallback(callback, "project,projects,rpcResponse", [null, [], null]);
            return;
        }
        if (!settings) {
            settings = {};
        ...
        var request = {
            params: params, clientContext: context,
            willHandleError:true, useSimpleHttp: true,
            containsCredentials: true, timeout: settings.timeout,
            actionURL: settings.projectLoaderURL || this.projectLoaderURL,
            callback: {target: this, methodName: "handleLoadProjectReply"}
        };

        // fill in unset request properties from settings.requestProperties
        var requestProperties = settings.requestProperties;
        if (requestProperties != null) {
            request = isc.addProperties({}, requestProperties, request);
        }

        // pass subset of settings as request params
        isc.addProperties(params, settings);
        delete params.omitLoadedDataSources;
        delete params.projectLoaderURL;
        delete params.requestProperties;
        delete params.timeout;

        // build param for omitDataSources, the DS to ignore
        this._addOmittedDataSources(settings, params);

        // handle boolean mistakenly passed to includeCustomizations instead of enum
        if (params.includeCustomizations == true) {
            params.includeCustomizations = "both";
        }

        this.sendRequest(request);
    },

    // add to request params omitDataSources, the list of DS IDs to ignore
    _addOmittedDataSources : function (settings, params) {
        // first, build a map of all DS explicitly specified by user to omit 
        var map = {}, omitted = settings.omitDataSources;
        if (isc.isAn.Array(omitted)) {
            for (var i = 0; i < omitted.length; i++) map[omitted[i]] = true;
        } else if (omitted) {
            map[omitted] = true;
        }

        // ignore the omitDataSources marker "*" if verifyDataSources is set
        var verifyDataSources = this._shouldVerifyLoadProjectDS(settings);
        if (verifyDataSources) delete map["*"];

        // if the special marker "*" is present, no need to go further
        if (map["*"]) {
            params.omitDataSources = "*";
            return;
        }

        // add to map all the DataSources currently loaded on the client
        var omitLoaded = settings.omitLoadedDataSources;
        if (omitLoaded || !verifyDataSources && omitLoaded != false) {
            var loadedDS = isc.DS.getRegisteredDataSourceObjects(true, true, true, true);
            for (var i = 0; i < loadedDS.length; i++) {
                var id = loadedDS[i].ID;
                if (id) map[id] = true;
            }
        }

        // now generate omitted param as comma-delimited list of DS IDs
        if (!isc.isAn.emptyObject(map)) {
            params.omitDataSources = isc.getKeys(map).join(",");
        }
    },

    
    _loadProjectUnableToLoad: "Unable to load any of the projects",

    //> @type LoadProjectErrorStatus
    // @value "timeout" server has not responded within configured request timeout
    // @value "noResponseFromURL" can't reach server at +link{Reify.serverURL}
    // @value "badReifyServerURL" no project loader at +link{Reify.serverURL}
    // @value "badCredentials" invalid +link{Reify.userName} or +link{Reify.password}
    // @value "projectNotFound" none of the requested projects were found
    // @value "servletError"
    // @visibility external
    //<
    ...
    // +link{loadProject()}.  Applies the following rules:<ul>
    // <li>reports "badCredentials" if the +link{RPCResponse.status} is
    // <code>STATUS_LOGIN_REQUIRED</code> or <code>STATUS_LOGIN_INCORRECT</code>
    // <li>reports "timeout" if the +link{RPCResponse.status} is
    // <code>STATUS_SERVER_TIMEOUT</code>
    // <li>reports "noResponseFromURL" if the +link{RPCResponse.httpResponseCode} is 0
    // <li>reports "badReifyServerURL" if the +link{RPCResponse.httpResponseCode} is 404 or
    // another code representing a transport error, such as 408 or 503
    // <li>reports "projectNotFound" if the +link{RPCResponse.httpResponseCode} is 500 and the
    // +link{RPCResponse.httpResponseText} contains the string
    // "Unable to load any of the projects"
    // <li>otherwise reports "servletError"</ul>
    //
    // @param rpcResponse (RPCResponse) server response
    // @return (String) the error status (null if no error)
    // @visibility external
    //<
    getLoadProjectErrorStatus : function (response) {
        var status = response.status;
        if (status < 0) {
            var errorCodes = isc.RPCResponse.errorCodes;

            switch (status) {
            case errorCodes.STATUS_LOGIN_REQUIRED:
            case errorCodes.STATUS_LOGIN_INCORRECT:
                return "badCredentials";

            case errorCodes.STATUS_SERVER_TIMEOUT:
                return "timeout";
            }

            var code = response.httpResponseCode,
                text = response.httpResponseText
            ;
            // failure is either a transport error, or error from the servlet
            if (this._isHttpResponseCommFailure(code)) {
                switch (code) { // transport error
                case 0:
                    return "noResponseFromURL";
                default:
                    return "badReifyServerURL";
                }
            } else {
                switch (code) { // servlet error
                case 500:
                    if (text && text.contains(this._loadProjectUnableToLoad)) {
    ...
    // +link{loadProject()}.  Calls +link{getLoadProjectErrorStatus()}.
    //
    // @param rpcResponse (RPCResponse) server response
    // @return (String) the error message (null if no error)
    // @visibility external
    //<
    getLoadProjectErrorMessage : function (response) {
        var errorCode = this.getLoadProjectErrorStatus(response);
        if (!errorCode) return;

        var errorKey = isc.RPCResponse.getErrorCodeKeyFromCode(response.status),
        ...
        switch (errorCode) {
        case "timeout":
            return "Server timed out trying to load the requested project(s): " +
                context.projectName;
        case "badCredentials":
            return "Invalid authorization for user '" + settings.USERNAME + "': " + errorKey;
        case "noResponseFromURL":
             return "Your browser disallowed a request to " + response.context.actionURL +
                ".  This may be because of browser extensions or network issues.";
        case "badReifyServerURL":
            return "No Reify project loader servlet present (or refused connection) at " +
                response.context.actionURL;
        case "projectNotFound":
            return "For user account '" + settings.USERNAME +
                "' couldn't find any of the requested projects: " + context.projectName;
        default:
            return data ? errorKey + ": " + data : "Unknown servlet error, HTTP CODE " +
                response.httpResponseCode + ": " + response.httpResponseText;
        }
    },

    
    handleLoadProjectReply : function (response, data, request) {
        var context = request.clientContext,
            settings = context.settings,
            callback = context.callback
        ;

        if (this.getLoadProjectErrorStatus(response)) {
            if (settings.willHandleError) {
                this.fireCallback(callback, "project,projects,rpcResponse", [null, [],
                                                                             response]);
            } else {
                this.logWarn("loadProject(): " + this.getLoadProjectErrorMessage(response));
            }
            return;            
        }
        ...
        try {
            ...
            if (settings.willHandleError) {            
                ...
                this.logWarn("loadProject(): invalid JSON data: " + data);
            }
            return;
        }
        this.logInfo("loadProject(): loaded " + data.length + " project(s)");

        this.loadProjectScreens(data, settings, callback, response);
    },

    loadProjectScreens : function (projects, settings, callback, response) {
        var projectCache = isc.Project._cachedProjects,
            dataContext = settings.dataContext,
            clobberDS = settings.clobberDataSources,
            allowPlaceholders = settings.allowPlaceholders,
            loadedProjects = [],
            firstProject,
            verificationReports = []
        ;
        // loop through each project loaded from the server
        for (var i = 0; i < projects.length; i++) {
            var newProject = isc.Project.create(projects[i]),
                oldProject = projectCache[newProject.projectName];
            // replace screen in global cache if it's from same project
            for (var j = 0; j < newProject.screens.length; j++) {
                var screen = newProject.screens[j];

                // add the settings dataContext and allowPlaceholders, if any to each screen
                if (dataContext != null) screen.dataContext = dataContext;
                if (allowPlaceholders != null) screen.allowPlaceholders = allowPlaceholders;

                var screenName = screen.ID;
                if (screenName in this._cachedScreens) {
                    var oldScreen = this._cachedScreens[screenName];
                    if (!oldProject || oldScreen != oldProject.screens.find("ID", screenName))
                    {
                        continue;
                    }
                    // warning parallels that from cacheScreens(); could be may an info log
                    this.logWarn("loadProject(): screen '" + screenName +
                                 "' is already cached for project '" + newProject.projectName +
                                 "'. Replacing.");
                }
                this._cachedScreens[screenName] = screen;
            }
            // update project registry so we can find project by name
            projectCache[newProject.projectName] = newProject;

            // collect first project and list of all projects for callback
            if (!firstProject) firstProject = newProject;
            loadedProjects.add(newProject);
        
            if (this._shouldVerifyLoadProjectDS(settings)) {
                var report = this.verifyProjectDataSources(projects[i]);
                verificationReports.addList(report);
            }
        }

        if (settings.verifyComponents && firstProject) {
            for (var i = 0; i < firstProject.screens.length; i++) {
                var entry = firstProject.screens[i];
                var screen =
                    this.createScreen(entry.ID, {
                        suppressAutoDraw: true, clobberDataSources: clobberDS
                    }, entry);
                var report = this.verifyComponents(screen, settings.verifyComponents);
                verificationReports.addList(report);
            }
        }

        // draw first screen of first project if so configured
        if (settings.drawFirstScreen && firstProject) {
            var topWidget = firstProject.createStartScreen({
                clobberDataSources: clobberDS                
            });
            if (topWidget && !topWidget.isDrawn()) topWidget.draw();
        }

        if (verificationReports.length > 0) {
            this.logVerificationsResult(verificationReports, {
                verifyAsError: settings.verifyAsError
            }, response);
        }

        // all required tasks are done, so trigger the callback
        this.fireCallback(callback, "project,projects,rpcResponse", [
            firstProject, loadedProjects, response
        ]);

    },

    verifyProjectDataSources : function (project) {
        var result = [];
        var dsList = project.dataSources;
        for (var i=0; i < dsList.length; i++) {
            var entry = dsList[i];
            var loaded = isc.DataSource.get(entry.ID);
            if (! loaded) {
                continue;
            }
            var coded = isc.eval(
                entry.code.replace('ID:"' + entry.ID + '"', 'ID:"isctmp_' + entry.ID + '"')
            );
            var report = isc.DataSource.verifyDataSourcePair(loaded, coded);
            coded.destroy();
            result.addList(report);
        }
        return result;
    },

    verifyComponents : function(screen, verificationProperties) {
        ...
        var result = [];
        for (var key in verificationProperties) {

            var componentId = key;
            var subComponentName = null;

            if (key.contains('.')) {
                var split = key.split('.');
                if (split.length != 2) {
                    result.add(buildMessage(key, 'ERROR',
                      'Component provided to verifyComponents takes an' +
                      'illegal form.  Should contain no more than one dot (".") separating ' +
                      ' the componentId from the subcomponent (e.g., formItem) name'));
                    continue;
                }
                componentId = split.first();
                subComponentName = split.last();
            }

            var component = screen.getByLocalId(componentId);
            if (component && subComponentName) {
                switch (component.getClassName()) {
                    case 'TabSet':
                        var tab = component.getTab(subComponentName);
                        
                        if (!tab && component.tabs) {
                           tab = component.tabs.find('autoID', subComponentName);
                           if (tab) {
                               tab._actualType = 'Tab';
                           }
                        }
                        ...
                        break;
                    case 'SectionStack':
                        var index = component.getSectionNumber(subComponentName);
                        component = (index == -1) ? null : component.sections[index];
                        break;
                    case 'DynamicForm':
                        component = component.getItem(subComponentName);
                        break;
                    default:
                        result.add(buildMessage(key,'ERROR',
                            'Unsupported component type "' + component.getClassName() +
                            '" verification requested.  Skipping.'));
                }
            }
            if (! component) {
                result.add(buildMessage(key, 'WARN', 'Component "' + key + '" declared in ' +
                    'verifyComponents not present in screen "' + screen.getLocalId() + '"'));
                continue;
            }

            
            var expectedType = verificationProperties[key];
            if ((component._actualType && component._actualType != expectedType) ||
                (component.getClass && !component.getClass().isA(expectedType))) {

                result.add(
                    buildMessage(key, 'WARN', 'Component "' + key +
                        ' declared in verifyComponents was not of expected type "' +
                        expectedType + '", instead is of type "' +
                        (component._actualType || component.getClassName()) + '"'
                    )
                );
            }
        }
    ...
    logVerificationsResult : function(reports, settings, response) {

        // If a boolean setting verifyAsError is set in the *Settings objects, instead of just
        // warning in the logs, problems from either verifyComponents or the DataSource
        // verifier appear, together, in an isc.warn() shown to the end user.
        var enableWarnDialog = isc.RPCManager.verifyAsError;
        if (settings && settings.verifyAsError != null) {
            enableWarnDialog = settings.verifyAsError;
        }
        var formatAsList = reports.length > 1;
        var messages = '';
        for (var i = 0; i < reports.length; i++) {
            var msg = reports[i].message;
            isc.logWarn(msg);
            messages += (formatAsList ? '<li>' : '') + msg;
        }
        if (formatAsList) {
            messages = '<ul>' + messages + '</ul>';
        }

        if (enableWarnDialog && !reports.isEmpty()) {
            isc.warn(messages);
        }

        
        if (response) {
            response.projectVerificationReports = reports;
        }
    },
    ...
    //> @attr createScreenSettings.allowPlaceholders (Boolean : null : IR)
    // Should +link{Placeholder,Placeholders} be rendered as placeholders?
    // If property is not set actual components are created instead of the Placeholders.
    // @visibility external
    //<

    //> @attr createScreenSettings.dataContext (DataContext: null : IR)
    // +link{DataContext} that will be provided to the top-level component as
    // +link{canvas.dataContext,dataContext} in the screen.
    // <P>
    // To understand how <code>dataContext</code> is used to automatically populate 
    // +link{DataBoundComponent,DataBoundComponents}, see +link{canvas.autoPopulateData}.
    // @group dataContext
    ...
    //> @attr createScreenSettings.clobberDataSources (Boolean : false : IRW)
    // Should DataSources referenced by the screen clobber existing, globally-bound DataSources
    // on the client when the screen is created?  The default of false means that any
    // DataSources defined in the screen will be discarded if they collide with existing,
    // globally-bound DataSources.
    // <p>
    // Note that here we consider a DataSource to be "globally bound" if it can be retrieved by
    ...
    //> @attr createScreenSettings.suppressAutoDraw (Boolean: false : IR)
    // If true, prevents any screen from being drawn when it's created, even if
    // there's an explicit +link{canvas.autoDraw}:true setting on it.
    // @visibility external
    //<    

    //> @attr createScreenSettings.htmlElement (DOMElement | String : null : IR)
    // Simplifies integrating a screen with an existing JavaScript app.  Does the following:
    // <ul>
    // <li>Sets +link{suppressAutoDraw}: true
    // <li>Sets +link{canvas.htmlElement} of the screen's top widget to the DOM element
    // <li>Sets the screen's top widget to have +link{canvas.position}: "relative"
    // <li>Draws the screen by calling +link{canvas.draw()} on the top widget</ul>
    // If you need to apply additional, custom configuration, for example setting
    // +link{canvas.htmlPosition}, then instead of using this property, you can call
    // +link{RPCManager.createScreen()} with +link{suppressAutoDraw}: true, and then
    ...
    //> @attr createScreenSettings.verifyAsError (boolean : null : IRW)
    // Enable +link{RPCManager.verifyAsError, verifyAsError} behavior only for
    // requests using these settings.
    // @see createScreenSettings.verifyAsError
    // @see loadProjectSettings.verifyAsError
    ...
    //> @attr createScreenSettings.verifyComponents (Map<String, String> : null : IR)
    // Enables verification that the created screen contains a component having a
    // <code>localId</code> equal to the given key, and that it is an instance (or subclass)
    // if the key's value.  Example:
    // <pre>
    ...
    // </pre>
    // Findings are always reported to the console, and may also be presented to the user
    // with a warning dialog by setting +link{createScreenSettings.verifyAsError} or
    // +link{RPCManager.verifyAsError}.
    // @see RPCManager.createScreen
    // @visibility external
    //<
    ...
    // @param [settings] (CreateScreenSettings | Array of String) widgets to allow to take their
    ...
    createScreen : function(screenName, settings, data, responseObj) {
        if (!screenName) {
            this.logWarn("No screen names passed in.");
            return null;
        }

        // use passed screen cache if present; avoids code duplication in caller
        data = data || this._cachedScreens[screenName];
        if (!data) { 
            this.logWarn("Asked to create screen '" + screenName +
                         "', but it's not cached - did you call cacheScreens() first?");
            return null;
        }

        
        var globals;
        if (isc.isAn.Object(settings)) globals = [];
        else                           globals = settings, settings = null;
        if (!isc.isAn.Array(globals)) globals = [globals];

        // If screen has been assigned a global dataContext by loadProject() and there is no
        // createScreen()-specific dataContext, use the global one
        if ((!settings || !settings.dataContext) && data.dataContext) {
            if (!settings) settings = {};
            settings.dataContext = data.dataContext;
        }

        if ((!settings || settings.allowPlaceholders == null) && data.allowPlaceholders != null) {
            if (!settings) settings = {};
            settings.allowPlaceholders = data.allowPlaceholders;
        }
        var result = this._makeScreen(null, {name: screenName, source: data.code}, null, null,
                                globals, settings);

        if (settings && settings.verifyComponents) {
            var report = this.verifyComponents(result, settings.verifyComponents);
            this.logVerificationsResult(report, settings, responseObj);
        }
    ...
    _makeScreen : function(rpcResponse, source, rpcRequest, callback, globals, screenSettings) {
        ...
        // rpcRequest.suppressAutoDraw sets prototype default for Canvas.autoDraw to false
        var origAutoDraw = isc.Canvas.getInstanceProperty("autoDraw"),
        ...
        // screenSettings.suppressAutoDraw forces ignoring even an explicit autoDraw:true
        var noAutoDraw;
        if (screenSettings && !isc.noAutoDraw &&
            (screenSettings.suppressAutoDraw || screenSettings.htmlElement))
        {
            noAutoDraw = isc.noAutoDraw = true;
        }

        // configure DataSource collision handling based on clobberDataSources setting
        if (isc.DS) {
            isc.DS._doNotClobber = !screenSettings || !screenSettings.clobberDataSources;
        }
        ...
        if (globals.length == 1 && globals[0] == _this.ALL_GLOBALS) {
            ...
            result = isc.Class.globalEvalWithCapture(source, function (globals, error) {
                ...
                if (noAutoDraw) delete isc.noAutoDraw;

                // restore default handling of DS collisions
                if (isc.DS) delete isc.DS._doNotClobber;

                // If caller provided dataContext, push it into the screen now
                if (screenSettings && screenSettings.dataContext) {
                    _screen.setDataContext(screenSettings.dataContext);
                }

                // if present, apply screenSettings.htmlElement to the screen
                if (screenSettings) {
                    _this._setScreenHtmlElement(_screen, screenSettings.htmlElement);
                }

                _this.fireCallback(callback, "screen,rpcResponse,suppressedGlobals", 
                    [_screen, rpcResponse, suppressedGlobals]);
            }, null, false);
            delete isc.trackCreateLevel;
            ...
            result = isc.Class.globalEvalAndRestore(source, globals, 
                ...
                var tmp = {},
                    keys = isc.getKeys(suppressedGlobals);
        ...
        // get top level view
        var result = isc.Canvas._getTopLevelWidget(result.globals);

        return result;
    },

    _setScreenHtmlElement : function (topWidget, htmlElement) {
        if (topWidget && htmlElement) {
            topWidget.setProperty("position", "relative");
            topWidget.setHtmlElement(htmlElement);
            topWidget.draw();
        }
    },
    ...
    cacheScreens : function(screenName, callback, locale, requestProperties) {
        ...
        request.callback = function (rpcResponse, data, rpcRequest) {
            ...
            isc.Class.globalEvalWithCapture(data, function (globals, error) {
                ...
                var json = _this._cacheScreensFromData(data);
    ...
    _cacheScreensFromData : function (data) {
        var json = isc.Class.evaluate(data);
        for (var i = 0; i < json.length; i++) {
            var screenName = json[i].screenName;
            var cached = this._cachedScreens[screenName];
            if (cached) {
                this.logWarn("Screen " + screenName + " is already cached. Replacing.");
            }
            this._cachedScreens[screenName] = {ID: screenName, code: json[i].source};
        }
        return json;
    },

    _buildCodeFromScreens : function(screens) {
        var sb = isc.SB.create();
        for (var i = 0; i < screens.length; i++) {
            sb.append(screens[i].source, "\n");
        }
        return sb.toString();
    },
    ...
    // +link{cacheScreens()} or similar (e.g. +link{loadProject()}), false otherwise.
    // 
    // @param screenName (String) name of the screen
    // @see loadScreenSettings.cacheScreen
    // @visibility external
    //<
    isScreenCached: function(screenName) {
        return screenName in this._cachedScreens;
    },
    ...
    _logPendingTransactions : function (category, logger) {
        if (!logger) logger = this;

        var output = [];
        for (var i = 0; i < this._transactions.length; i++) {
            output[i] = this.echo(this._transactions[i]);
        }
        logger.logDebug(output.length + " pending transaction(s):\n" +
                        output.join(", "), category);
    }
...
isc.RPCManager.addClassProperties({
    loginFailureFieldDefaults: {
        name: "loginFailure",
        type: "blurb", textAlign: "center", wrap: true,
...
// <li>Response data: new record as stored, as an Object or Array of one Object
// </ul>
//
// <b>update</b>
// <ul>
...
// an Object or Array of one Object
// </ul>
//
// <b>custom</b>
// <ul>
...
// +explorerExample{javabeanWizard,Reify Javabean Wizard} and the Batch DataSource 
...
// <p>
// <b>Reify:</b> The DataSource Wizards in Reify are pluggable; we provide wizards for SQL and
// Hibernate DataSources, and it is easy to write a new wizard to integrate your custom
// DataSource into Reify.  +explorerExample{sqlWizard,SQL Wizard screenshots},
...
// <b>Simplicity</b>
// <p>
// With the SmartClient SQL DataSource, simple CRUD connectivity can easily be set up in a
// way that requires zero server side code.  Only a DataSource
// descriptor (.ds.xml file) needs to exist; this descriptor can include field-level
// details explicitly, if you choose to provide them, or can be inferred from your schema
// via +link{DataSource.autoDeriveSchema}.  The descriptor actually serves double duty by also
// providing the configuration for UI components - in other words, this is information that you
// would need to express anyway.
// <p>
...
// other unnecessary scaffolding.  MyBatis avoids some of this scaffolding, but requires every
...
//
// @title SQL DataSource vs JPA, EJB, MyBatis and other technologies
// @treeLocation Concepts/Persistence Technologies
// @visibility external
//<
...
//>	@class Project
//
// Represents a Reify project loaded from the server via +link{RPCManager.loadProject()}.  A
// project contains cached screens and +link{DataSource,DataSources} that can be used to create
// actual screens by calling +link{project.createScreen()} or +link{project.createStartScreen()}.
// @treeLocation Client Reference/RPC
// @visibility external
//<
isc.defineClass("Project").addClassProperties({

    _cachedProjects: {},

    //> @classMethod project.get()
    // Returns a cached project given its name.
    // @param projectName (String) name of project to retrieve
    // @return (Project) cached project, or null if it's not cached
    // @visibility external
    //<
    get : function (projectName) {
        var project = this._cachedProjects[projectName];
        if (!project) {
            this.logInfo("get(): no project named '" + projectName + "' was ever loaded");
        }
         return project;
    }
...
isc.Project.addProperties({

    // the projectName isn't a valid ID, but apply it to log messages
	logMessage : function (priority, message, category, timestamp) {
        if (!this.getID()) message = this.projectName + ":" + message;
        return this.Super("logMessage" ,arguments);
    },
    
    //> @method project.createScreen()
    // Creates a screen from screen definitions cached in the project.
    // @param screenName (String) name of screen to create
    // @param [settings] (CreateScreenSettings | Array of String) widgets to allow to take their
    //                                                      global IDs, or a widget remap config
    // @return (Canvas) last top-level widget in the screen definition
    // @see RPCManager.createScreen()
    ...
    createScreen : function(screenName, settings, responseObj) {
        if (this.destroyed) {
            this.logWarn("createScreen(): project has been destroyed");
            return;
        }
        if (!screenName) {
            this.logWarn("createScreen(): no screen names passed in");
            return null;
        }

        var data = this.screens.find("ID", screenName);
        if (!data) { 
            this.logWarn("createScreen(): asked to create screen '" + screenName +
                         "', but it's not cached - Are you sure it's in this project?");
            return null;
        }

        this.runCustomizations();

        // make the screenId and projectId available globally during creation of the screen
        isc.Project._currentScreenId = screenName;
        isc.Project._currentProjectId = this.projectName;
        try {
            var screen = isc.RPCManager.createScreen(screenName, settings, data, responseObj);
            screen._screenId = screenName, screen._projectId = this.projectName;
        } finally {
            delete isc.Project._currentScreenId;
            delete isc.Project._currentProjectId;
        }

        return screen;
    },

    // evaluate any project customizations
    runCustomizations : function () {
        var customizations = this.customizations;
        if (customizations) try {
            isc.ClassFactory.ignoreDuplicateClass = true;
            if (isc.DS) isc.DS._doNotClobber = true;
            
            isc.Class.evaluate(customizations);

            this.logInfo("successfully evaluated your customizations", 
                         "projectCustomizations");
        } catch (e) {
            if (this.logIsInfoEnabled("projectCustomizations")) {
                this.logInfo("error evaluating your customizations: '" + customizations +
                             "':\n" + e, "projectCustomizations");
            }
        } finally {
            delete isc.ClassFactory.ignoreDuplicateClass;
            if (isc.DS) delete isc.DS._doNotClobber;
        }
    ...
    createStartScreen : function (settings, responseObj) {
        if (this.destroyed) {
            this.logWarn("createStartScreen(): project has been destroyed");
            return;
        }
        if (!this.screens.length) {
            this.logWarn("createStartScreen(): project has no screens");
            return;
        }
        return this.createScreen(this.screens[0].ID, settings, responseObj);
    },

    //> @method project.getScreenNames()
    // Return the names of the screens cached in this project.
    // @return (Array of String) names of cached screens
    // @see createScreen()
    // @visibility external
    //<
    getScreenNames : function () {
        return this.screens.map(function (screen) {return screen.ID;});
    },

    //> @method project.getDataSourceNames()
    // Return the names of the +link{DataSource,DataSources} cached in this project.
    // @return (Array of String) names of cached DataSources
    // @visibility external
    //<
    getDataSourceNames : function () {
        return this.dataSources.map(function (ds) {return ds.ID;});
    },

    //> @method project.getDataSource()
    // Returns an instance of the requested +link{DataSource} by creating it from the project's
    // cache.  If the ID is not globally bound, the framework will globally bind the instance
    // before returning it.
    // <p>
    // Note that when a screen cached in the project is created, all project DataSources in the
    // screen will be automatically instantiated from the project cache, so this method need
    // not be called before creating a screen just to ensure its DataSources are available.
    // @param ID (Identifier) ID of the DataSource to create
    // @return (DataSource) requested DataSource instance
    // @visibility external
    //<    
    getDataSource : function (ID, skipGlobalId) {
        var data = this.dataSources.find("ID", ID);
        if (!data) {
            this.logWarn("getDataSource(): asked to get DataSource '" + ID +
                         "', but it's not cached.  Are you sure it's in this project?");
            return null;
        }
        ...
        var suppressGlobal = true;

        if (!skipGlobalId) {
            var existingValue = window[ID];
            if (existingValue) {
                if (existingValue == isc.DS.get(ID)) {
                    this.logInfo("getDataSource(): existing global DataSource with ID '" +
                                 ID + "' will not be replaced");
                } else if (isc.isA.DataSource(existingValue)) {
                    this.logWarn("getDataSource(): possible error; existing global with ID '" +
                                 ID + "is a DataSource, but is not registered under that ID");
                } else {
                    this.logWarn("getDataSource(): probable error; existing global with ID '" +
                                 ID + "is not a DataSource");
                }
            } else {
        ...
        isc.DS.suppressIDRegistration = !!suppressGlobal;
        try {
            return isc.Class.evaluate(data.code);
        } finally {
            delete isc.DS.suppressIDRegistration;
        }
    },

    //> @method project.destroy()
    // Releases cached screens and +link{DataSource,DataSources} associated with this project,
    // and unregisters it so +link{Project.get()} no longer can find it by name.  After
    // destroying a project, it is an error to call any instance method on it.
    // @visibility external
    //<    
    destroy : function () {
        var projectName = this.projectName,
            projectCache = isc.Project._cachedProjects,
            cachedProject = projectCache[projectName];
        if (cachedProject == this) delete projectCache[projectName];

        // ensure that no more Screens or DataSources can be created from this project
        for (var i = 0; i < this.screens.length;     i++) delete this.screens[i].code;
        for (var i = 0; i < this.dataSources.length; i++) delete this.dataSources[i].code;
...
// DataSourceRetriever instances are emitted into screen component XML to avoid duplicating DS
// definitions.  Creating a DSR will create the referenced DataSources from project cache.
isc.defineClass("DataSourceRetriever").addClassMethods({

    create : function (properties) {
        var dsNames     = properties.dataSources,
            projectName = properties.projectName
        ;
        // create the DataSources listed in retriever
        var project = isc.Project.get(projectName);
        if (project) {
            var dataSources = project.dataSources;
            for (var i = 0; i < dsNames.length; i++) {
                var ID = dsNames[i],
                    data = dataSources.find("ID", ID);
                if (!data) {
                    if (!isc.DataSource.get(ID)) {
                        this.logWarn("can't create DataSource '" + ID + "' for project '" +
                            projectName + "' because DataSource not cached in the project.");
                    }
                    continue;
                }
                isc.Class.evaluate(data.code);
            }
        }
    }
...
if (isc.RPCManager.supportsPostMessageRelogin) {
    isc.RPCManager.setupReloginListener();
}
